"""Project scaffolding — the pure logic behind ``kodo project new`` / ``init`` (A7).
...
#: The project-local library directory ``kodo project init`` scaffolds (holds this project's models).
LOCAL_LIBRARY = "library"


def kodo_repo_root() -> Path:
...
def render_pyproject(name: str, mcp: list[tuple[str, str]], mlx: bool, extras: list[str] | None = None) -> str:
    ...
    deps = [f"kodo{extras_spec}", *pip_deps_from_mcp(mcp)]
    dep_lines = "".join(f"    {json.dumps(d)},\n" for d in deps)
    kodo_root = json.dumps(str(kodo_repo_root()))
    return (
        "[project]\n"
