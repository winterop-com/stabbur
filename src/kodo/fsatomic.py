"""Atomic, durable file writes for kodo's small config/index files.

One implementation of the write-temp-then-rename pattern used by every small file kodo can't
afford to corrupt (tags, machine config, ``mcp.json``): stage into a **per-write unique** temp
file (so a concurrent CLI + ``kodo serve`` writer can't clobber each other's staging), fsync it,
