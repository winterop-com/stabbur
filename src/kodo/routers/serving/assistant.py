...
kodo stays domain-generic — it never interprets these fields. ``GET /api/assistant`` returns the
...
can show a live connection state without kodo knowing what "connected" means for the domain.
"""
...
from kodo.routers.serving._base import router
...
def _killpg(proc: asyncio.subprocess.Process) -> None:
    ...
    Mirrors :mod:`kodo.runtime.supervisor`: a plain ``proc.kill()`` signals only the direct child, so
...
class AssistantResponse(BaseModel):
    """The project's ``[assistant]`` metadata, echoed for UI clients (kodo never interprets it).

    Mirrors :class:`kodo.project.AssistantInfo`'s public fields minus ``verify`` (an execution
...
async def assistant(request: Request, verify: bool = False) -> AssistantResponse:
    ...
    Read-only echo — kodo never interprets these fields. ``can_verify`` reports whether a live
...
def _template_argv(argv: list[str], info: AssistantInfo) -> list[str]:
    ...
    the allowed set + regex are imported from :mod:`kodo.project` so there's one source of truth.
    """
