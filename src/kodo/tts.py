...
def synthesize(text: str, out_path: Path | None = None) -> Path:
    """Generate a speech WAV from ``text`` with the default OuteTTS models.

    Returns the path to the written WAV (a temp file if ``out_path`` is omitted).

    Raises:
        RuntimeError: If ``llama-tts`` is missing or synthesis fails.
    """
