"""The catalog of voice (TTS / STT) models kodo knows how to run.
...
class Backend(StrEnum):
    ...
    kokoro_onnx = "kokoro-onnx"  # cross-platform ONNX; kodo's built-in Kokoro path
...
class VoiceModel(BaseModel):
    """A voice model kodo can run: how to find it, run it, and drive its voice."""

    model_config = ConfigDict(frozen=True)

    id: str  # kodo's short id, e.g. "kokoro", "dia", "whisper"
    display_name: str
    repo: str  # the Hugging Face repo kodo prefers (MLX variant on Apple Silicon)
    kind: VoiceKind
    backend: Backend
    description: str = ""
...
BUILTIN: tuple[VoiceModel, ...] = (
    VoiceModel(
        ...
        description="Small, fast multi-voice TTS with 54 built-in named voices. kodo's default "
...
def get(model_id: str) -> VoiceModel | None:
    """Look up a known voice model by kodo id."""
