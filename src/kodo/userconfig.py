"""Durable machine-level config — the write-mode home for ``kodo config`` / ``kodo setup``.

Three config layers, kept distinct on purpose:

- a project's ``kodo.toml`` — portable, per-project, committed (model + tools + libraries);
- **this** — per-machine defaults (the library location, a default model to use outside a
  project), persisted at ``$XDG_CONFIG_HOME/kodo/config.toml`` so a fresh machine needs no
  shell export or cwd ``.env``;
- ``~/.kodo`` — ephemeral runtime state (pidfiles/logs), which must never travel.

It's a flat TOML file of scalar ``field = value`` lines whose keys are :class:`kodo.config.Settings`
field names, fed in as the lowest-priority settings source (a real ``KODO_*`` env var or a project
``kodo.toml`` still wins). Reads go through :mod:`kodo.config`; writes go through :func:`set_value`.
"""
...
def config_dir() -> Path:
    """Kodo's XDG config dir (``$XDG_CONFIG_HOME/kodo``, else ``~/.config/kodo``)."""
...
def set_value(field: str, value: str) -> Path:
    ...
    touches disk so a bad value never corrupts the file (mirrors how ``kodo.toml`` is edited).
