...
a crashed kodo (orphaned, holding tens of GB), grandchildren could survive ``terminate()``, and
...
* **Parent-death recovery** — each runtime records a ``meta.json`` (its pid/pgid/cmd + the pid of
  the kodo that owns it) under ``<runtime_state_dir>/<id>/``. On a graceful exit an ``atexit``
  hook stops everything; for an *ungraceful* death (SIGKILL/OOM), the child orphans momentarily
  and is reclaimed by :func:`sweep_orphans` on the next kodo start — which only kills a runtime
  whose owning kodo is **gone** and whose live cmdline still matches (guarding against PID reuse).
...
POSIX only (macOS + Linux); kodo does not serve on Windows.
"""
...
def _write_meta(state_dir: Path, proc: subprocess.Popen[bytes], cmd: list[str], port: int, name: str) -> None:
    try:
        ...
        pgid = proc.pid
    meta = {
        "owner_pid": os.getpid(),  # the kodo that spawned this; a dead owner marks an orphan
        "pid": proc.pid,
        "pgid": pgid,
        "cmd": cmd,
        "port": port,
...
def sweep_orphans() -> list[int]:
    """Kill runtimes left behind by a crashed kodo and clean their state dirs.

    Only reaps a runtime whose **owning kodo is gone** (so it never touches a runtime a
    concurrently-running kodo still owns) and whose live cmdline still matches its meta (so a
    ...
    for entry in root.iterdir():
        ...
        if owner_pid and _pid_alive(owner_pid):
            continue  # a live kodo still owns this runtime — leave it be
