...
kodo reads and writes the same ``mcpServers`` format Claude Desktop / Claude Code / Cursor use,
...
- **global** — ``~/.config/kodo/mcp.json`` (machine-wide defaults, e.g. what free-play chat gets).

A server entry is ``{ "command": ..., "args": [...], "env": {...} }`` keyed by name; kodo's bundled
first-party servers are entered by their package (``kodo-mcp-*``). :func:`resolve` returns the
effective set — global first, then project (a project name overrides a global one); the CLI's
``--mcp`` is layered on top by the caller. This is deliberately separate from ``kodo.toml``, which
...
def global_path() -> Path:
    """The machine-global config (``<XDG config>/kodo/mcp.json``)."""
