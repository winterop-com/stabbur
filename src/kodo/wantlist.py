...
The library is already a manifest — every pull records its source in a ``.kodo/metadata.json``
sidecar, and Ollama models are reconstructable from their manifests. ``kodo library manifest``
reads that back into a portable, human-editable list of ``[[model]]`` entries (source + name +
format); ``kodo library sync`` diffs such a file against the library and re-pulls what's missing
...
file, mirroring the one-parser-one-writer discipline of ``kodo.toml`` (:mod:`kodo.project`).
"""
...
from kodo.models import ModelFormat, ModelSource, PullResult

_HEADER = (
    "# kodo want list - models to keep in this library.\n"
    "# Regenerate with:  kodo library manifest --save <file>\n"
    "# Re-download with: kodo library sync <file>\n"
)
...
def _sidecar_meta(model: LibraryModel) -> dict[str, Any]:
    """Read a directory model's ``.kodo/metadata.json`` (records its source), or ``{}`` if absent."""
...
def _voice_id(repo: str) -> str | None:
    ...
    from kodo.voice import registry  # noqa: PLC0415 - keep voice deps lazy
...
def entry_for(model: LibraryModel) -> WantModel | str:
    ...
    * Everything else is a Hugging Face repo — its library name *is* the repo id. LM Studio backups
    ...
    if model.voice_kind:
        ...
        if vid is None:
            return f"voice model {model.name!r} — re-add with: kodo library pull voice <id>"
...
def render(entries: list[WantModel], comments: list[str] | None = None) -> str:
    """Render a want list as TOML — the single writer for this file (kept minimal, like ``kodo.toml``).
...
def pull_entry(entry: WantModel, library_root: Path | None) -> PullResult:
    ...
    Routes every source through :func:`kodo.catalog.pull` so sync reuses the exact pull machinery
    the CLI ``library pull`` command does — no re-implementation.
    """
    from kodo import catalog  # noqa: PLC0415 - lazy: keeps this module's import light
