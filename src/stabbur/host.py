"""OS-aware helpers so stabbur runs cleanly on both macOS and Linux.

stabbur serves on POSIX only (macOS + Linux; see :mod:`stabbur.supervisor`). The two
...
place to teach stabbur a new OS.
"""
...
# Runtime binaries stabbur spawns / shells out to, and how to install them per OS.
# MLX is Apple-Silicon-only, so its hint is the same everywhere (an install hint
# for a runtime that only exists on the Mac).
# Two install shapes, because the right command depends on how stabbur itself was installed:
...
def package_hint(package: str) -> str:
    ...
    Covers the CLI tools stabbur shells out to that ship in every mainstream package
...
def install_hints() -> dict[str, str]:
    """Install hints for the runtime binaries stabbur spawns, keyed by executable name.

    Tailored to the running OS so ``stabbur doctor`` and runtime errors never point a
