"""Discovery of running ``stabbur serve`` processes, so ``stabbur chat`` can reuse a loaded model.

Each ``stabbur serve`` writes a small record (its base URL + the model it locked + its pid) under the
machine runtime dir; ``stabbur chat``, given no explicit ``--server``, looks here for a **live** server
...
Only **unauthenticated** serves register — a loopback ``stabbur serve`` with no bearer token. A serve
that needs auth (non-loopback, or an explicit ``STABBUR_AUTH_TOKEN``) is left out, since ``stabbur chat``
...
class ServeRecord(BaseModel):
    """A running ``stabbur serve``'s discovery record."""
...
def register(base_url: str, model: str | None) -> Path | None:
    """Record this ``stabbur serve`` (its base URL + locked model) so ``stabbur chat`` can find it.
...
def unregister() -> None:
    """Remove this process's serve record (called when ``stabbur serve`` stops)."""
    try:
...
def discover(model: str) -> ServeRecord | None:
    ...
    for record_file in directory.glob("*.json"):
        ...
        # older stabbur) or whose live cmdline no longer matches is stale; sweep it like a dead pid.
