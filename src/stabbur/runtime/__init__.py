...
from stabbur import capabilities, host
from stabbur.config import debug_enabled, get_settings, pinned_runtime_port
...
# OS-tailored install hints (see stabbur.host) so a missing binary points macOS users
...
def runnable_error(model: LibraryModel) -> str | None:
    """Return why ``model`` can't be run by stabbur, or ``None`` if it's runnable.
    ...
    stabbur runs generative GGUF/MLX models; it rejects non-generative (embedding/
    ...
    if not model.generative:
        return f"{model.name!r} is not a chat model ({model.model_format.value}); stabbur runs generative LLMs only"
    if model.is_ollama:
        return f"{model.name!r} is an Ollama model; run it via Ollama, not stabbur (stabbur runs GGUF/MLX)"
    if model.model_format is ModelFormat.safetensors:
        return (
            f"{model.name!r} is safetensors (a convert/fine-tune source), not directly runnable; "
            "pull a GGUF or MLX build to run it"
...
def _early_exit_error(cmd: list[str], code: int | None, log_path: Path | None, port: int) -> RuntimeError:
    ...
    if "address already in use" in low or "couldn't bind" in low or "bind http server socket" in low:
        msg += f"; port {port} is already in use — another stabbur runtime may be running"
    if tail:
        msg += f"\n--- runtime log ---\n{tail}"
...
# A runtime handle is a supervised process (:class:`stabbur.supervisor.RuntimeHandle`) — the CLI holds
...
def resolve_binary(name: str) -> str | None:
    """Locate a runtime executable: stabbur's own environment first, then PATH.

    The MLX runtimes are stabbur ``extras``, so `uv tool install -e ".[mlx]"` puts
    ``mlx_lm.server`` in stabbur's tool environment — where uv exposes only stabbur's *own*
    entry points, leaving the runtime off PATH. Looking beside the running interpreter
    first means installing the extra "into stabbur" works without polluting a global PATH
    (the same reason :func:`stabbur.tools._bin_dir` exists for the bundled MCP servers).
...
def generate(
    ...
    (a running ``stabbur serve`` or any OpenAI-compatible server), attach to that server's ``/v1``
    ...
    from stabbur import agent  # noqa: PLC0415 - avoid import cycle at module load
...
def _chat(
    ...
    from stabbur.runtime import sampling  # noqa: PLC0415 - avoid import cycle at module load
