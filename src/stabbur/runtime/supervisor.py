...
a crashed stabbur (orphaned, holding tens of GB), grandchildren could survive ``terminate()``, and
...
* **Parent-death recovery** — each runtime records a ``meta.json`` (its pid/pgid/cmd + the pid of
  the stabbur that owns it) under ``<runtime_state_dir>/<id>/``. On a graceful exit an ``atexit``
  hook stops everything; for an *ungraceful* death (SIGKILL/OOM), the child orphans momentarily
  and is reclaimed by :func:`sweep_orphans` on the next stabbur start — which only kills a runtime
  whose owning stabbur is **gone** and whose live cmdline still matches (guarding against PID reuse).
...
POSIX only (macOS + Linux); stabbur does not serve on Windows.
"""
...
def _write_meta(state_dir: Path, proc: subprocess.Popen[bytes], cmd: list[str], port: int, name: str) -> None:
    try:
        ...
        pgid = proc.pid
    meta = {
        "owner_pid": os.getpid(),  # the stabbur that spawned this; a dead owner marks an orphan
        "pid": proc.pid,
        "pgid": pgid,
        "cmd": cmd,
        "port": port,
...
def sweep_orphans() -> list[int]:
    """Kill runtimes left behind by a crashed stabbur and clean their state dirs.
