...
``generation_config.json``); stabbur mirrors that so a model behaves the way its
...
# stabbur's own sampling defaults, applied when a model ships no recommendation of its own
...
# stabbur can *show* and send, instead of silently inheriting whatever runtime happens to serve
...
def defaults() -> "ModelSampling":
    """Stabbur's sampling defaults — what a model with no recommendation of its own gets."""
...
def recommended(model: LibraryModel) -> ModelSampling:
    ...
    Every field always comes back set: a value the model recommends, else stabbur's own
