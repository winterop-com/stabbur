...
(``~/.cache/huggingface/hub``), *not* in stabbur's portable library — so they don't travel with
...
``stabbur voice list`` and for importing cache -> library.
"""
...
from stabbur.voice.registry import BUILTIN, VoiceModel
...
class VoicePresence(BaseModel):
    ...
    def available(self) -> bool:
        """Whether the model is present anywhere stabbur can reach it."""
