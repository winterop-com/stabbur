...
def pull_to_library(
    ...
    This is what ``stabbur library pull voice <id>`` calls, so voice models land in the project-local
...
def _other_library_with(repo: str, target: Path) -> Path | None:
    ...
    Uses :func:`stabbur.library.roots` to find the libraries in scope, skipping ``target`` itself, so
    a model already on the shared drive is copied rather than re-downloaded.
    """
    from stabbur.library import roots  # noqa: PLC0415 - lazy: avoid importing library on every voice import
