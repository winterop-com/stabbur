...
The TTS backends produce WAV. Clients (the ``/v1/audio/speech`` endpoint, ``stabbur voice speak``)
often want a compressed format instead — mp3 for size, opus for streaming, flac for lossless.
Rather than teach each backend a second encoder, stabbur synthesizes to WAV once and transcodes
...
def is_supported(fmt: str | None) -> bool:
    """Whether ``fmt`` is a format stabbur can export."""
