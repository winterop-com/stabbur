...
class LibraryNotConfigured(RuntimeError):
    """No library location is configured — stabbur can't do anything useful without one.

    The shared/default library must be set explicitly (``STABBUR_LIBRARY_ROOT``) rather than
    silently falling back to a CWD-relative ``./data`` (which is meaningless for a globally
    installed CLI). A project (``stabbur.toml``) that lists its own ``libraries`` counts as
    configured. Carries a ready-to-print, actionable message.
    """

    HINT = (
        "No library configured. Point stabbur at your model library — set STABBUR_LIBRARY_ROOT:\n"
        "  export STABBUR_LIBRARY_ROOT=/path/to/your/library   # an external drive is ideal\n"
        "or run `stabbur project init` to scaffold a project with its own library."
    )
...
def roots(settings: Settings | None = None) -> list[Path]:
    ...
    A project (``stabbur.toml``) may list ``libraries`` — paths relative to the project
...
def default_root(settings: Settings | None = None) -> Path:
    """The machine's default library (``STABBUR_LIBRARY_ROOT`` / ``@shared``), or raise.
