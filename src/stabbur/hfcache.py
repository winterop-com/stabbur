...
stabbur's library — e.g. an mlx-audio model that fetches a companion codec
...
imported — hence :func:`configure` is called from :mod:`stabbur` (the package ``__init__``)
...
def drive_cache_dir() -> Path | None:
    ...
    """
    try:
        from stabbur.config import get_settings  # noqa: PLC0415 - avoid import cycle at module load

        root = get_settings().library_root
    except Exception:  # noqa: BLE001 - never let config errors break importing stabbur
        return None
