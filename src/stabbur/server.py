...
def parse_model_listing(data: object) -> list[UpstreamModel]:
    ...
    same way the serving path does — notably ``stabbur doctor``'s backend check, which must never
...
class UpstreamManager:
    ...
    The ``serve --upstream`` backend: stabbur's agent loop, MCP tools, confirm gate, and the
    web UI run here while the models run on the remote box (a llama-server in router mode,
    LM Studio, mlx-lm, another stabbur serve). Duck-types :class:`ServerManager`'s read surface
    ...
    def _warmup(self, model_id: str) -> None:
        ...
        the user's first message is what makes a model switch real: without it stabbur reports the
    ...
    def load_by_name(self, name: str, *, warmup: bool = True) -> None:
        ...
        recorded, so stabbur never reports a model as ready that the remote is not actually serving.
        ...
        # rather than point stabbur at a model the remote never loaded. ``_loading`` is what the
