...
guarantee. Set ``STABBUR_WEB_ALLOW_PRIVATE=1`` to intentionally read an internal/localhost
host. A per-fetch navigation timeout bounds hangs and the returned Markdown is
length-capped (``STABBUR_WEB_MAX_CHARS``).
...
Run standalone over stdio: ``stabbur-mcp-web`` (or ``python -m stabbur.mcp_servers.web``). Point stabbur at it
with ``stabbur mcp add web`` (or ``stabbur chat --mcp stabbur-mcp-web``).
"""
...
mcp: FastMCP = FastMCP("stabbur-web")


class Settings(BaseSettings):
    """Runtime config, from ``STABBUR_WEB_*`` env vars (all optional)."""

    model_config = SettingsConfigDict(env_prefix="STABBUR_WEB_")
    ...
    # Playwright wait strategy; a Literal so an invalid STABBUR_WEB_WAIT_UNTIL is rejected at startup.
    wait_until: Literal["load", "domcontentloaded", "networkidle", "commit"] = "load"
    allow_private: bool = False  # opt in to private/loopback hosts (internal servers)
    user_agent: str = "stabbur-mcp-web/0.1 (+https://github.com/winterop-com/stabbur)"
...
def _guard_url(url: str) -> None:
    ...
    ``STABBUR_WEB_ALLOW_PRIVATE=1`` skips the address check (but the scheme is still enforced).
    """
    parsed = urlparse(url)
