...
its output back. The sandbox is :mod:`stabbur_sandbox` (shared with the benchmark). Requires a
running Docker daemon; without one the tool returns a clear error instead of executing.

Run standalone over stdio: ``stabbur-mcp-exec`` (or ``python -m stabbur.mcp_servers.exec``). Point stabbur at it
with ``stabbur chat --mcp exec`` or a ``[[mcp]]`` block.
"""
...
from stabbur_sandbox import DockerError, run_code

mcp: FastMCP = FastMCP("stabbur-exec")
