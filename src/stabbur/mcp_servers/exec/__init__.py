"""Stabbur MCP server: a locked-down Python code sandbox (calculator / scratchpad).

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) so both the
``stabbur-mcp-exec`` console script and ``python -m stabbur.mcp_servers.exec`` resolve against the package root.
"""
