"""Stabbur MCP server: web search returning titled results (title, url, snippet).

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) from ``app`` so
both the ``stabbur-mcp-search`` console script and ``python -m stabbur.mcp_servers.search`` resolve
