...
title, URL, and snippet — for a model to then ``read_url`` (pairs with ``stabbur-mcp-web``).

Backends (``STABBUR_SEARCH_BACKEND``, default ``auto``):

* ``duckduckgo`` — no API key; scrapes the DuckDuckGo HTML endpoint. The zero-config default.
* ``brave`` — the Brave Search API; needs ``STABBUR_SEARCH_BRAVE_KEY``.
* ``exa`` — the Exa API; needs ``STABBUR_SEARCH_EXA_KEY``.
...
Run standalone over stdio: ``stabbur-mcp-search`` (or ``python -m stabbur.mcp_servers.search``). Point stabbur
at it with ``stabbur mcp add search`` (or ``stabbur chat --mcp stabbur-mcp-search``).
"""
...
mcp: FastMCP = FastMCP("stabbur-search")


class Settings(BaseSettings):
    """Runtime config, from ``STABBUR_SEARCH_*`` env vars (all optional)."""

    model_config = SettingsConfigDict(env_prefix="STABBUR_SEARCH_")
    ...
    user_agent: str = "Mozilla/5.0 (compatible; stabbur-mcp-search/0.1; +https://github.com/winterop-com/stabbur)"
...
async def _search_brave(query: str, limit: int) -> list[SearchResult]:
    if not settings.brave_key:
        raise ValueError("the Brave backend needs STABBUR_SEARCH_BRAVE_KEY (or use the default duckduckgo backend)")
    async with httpx.AsyncClient(
        timeout=settings.timeout_seconds,
...
async def _search_exa(query: str, limit: int) -> list[SearchResult]:
    if not settings.exa_key:
        raise ValueError("the Exa backend needs STABBUR_SEARCH_EXA_KEY (or use the default duckduckgo backend)")
    async with httpx.AsyncClient(
        timeout=settings.timeout_seconds,
...
async def search(query: str, max_results: int = 0) -> str:
    ...
    Brave/Exa API when a ``STABBUR_SEARCH_*`` key is configured.
    """
    limit = max_results if max_results and max_results > 0 else settings.max_results
    backend = resolve_backend()
    fn = _BACKENDS.get(backend)
    if fn is None:
        raise ValueError(f"unknown STABBUR_SEARCH_BACKEND {backend!r}; use one of: auto, duckduckgo, brave, exa")
    try:
