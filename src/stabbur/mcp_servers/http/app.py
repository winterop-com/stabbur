...
with a clear "no hosts allowlisted" message until you opt hosts in via ``STABBUR_MCP_HTTP_ALLOWLIST``.
...
the auto-follow is disabled. Set ``STABBUR_MCP_HTTP_ALLOW_PRIVATE=1`` to intentionally reach an
...
caveat ``stabbur-mcp-web`` notes for its browser path) — we resolve, vet, and pin once, which is the
strong, practical guarantee.

Run standalone over stdio: ``stabbur-mcp-http`` (or ``python -m stabbur.mcp_servers.http``). Point stabbur at it
with ``stabbur mcp add http`` (or ``stabbur chat --mcp stabbur-mcp-http``).
"""
...
mcp: FastMCP = FastMCP("stabbur-http")


class Settings(BaseSettings):
    """Runtime config, from ``STABBUR_MCP_HTTP_*`` env vars (all optional).

    ``allowlist`` is empty by default so the tool is fail-closed (deny all) until hosts are opted
    in. Comma-separated in the env var, e.g. ``STABBUR_MCP_HTTP_ALLOWLIST=example.com,api.acme.io``.
    """

    model_config = SettingsConfigDict(env_prefix="STABBUR_MCP_HTTP_")
    ...
    user_agent: str = "stabbur-mcp-http/0.1 (+https://github.com/winterop-com/stabbur)"
...
def _vet_hop(url: str) -> tuple[str, dict[str, str], dict[str, str]]:
    ...
    if not settings.allowlist:
        raise HttpToolError(
            "no hosts allowlisted — set STABBUR_MCP_HTTP_ALLOWLIST to opt hosts in (deny-all by default)"
        )
...
async def http_get(url: str, headers: dict[str, str] | None = None) -> HttpResponse:
    ...
    so nothing is reachable until you configure ``STABBUR_MCP_HTTP_ALLOWLIST``). Private/loopback/
    internal addresses are refused, and redirects are re-checked at every hop. The body is capped
    (``STABBUR_MCP_HTTP_MAX_BYTES``) and marked ``truncated`` if it was cut. Blocks, timeouts, and
