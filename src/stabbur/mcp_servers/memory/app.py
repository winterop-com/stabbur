...
lives in the library by default (``<STABBUR_LIBRARY_ROOT>/.stabbur/memory/notes.json``), so it
travels with the drive — see :mod:`stabbur.mcp_servers.memory.core` for the location rules.

Run standalone over stdio: ``stabbur-mcp-memory`` (or ``python -m stabbur.mcp_servers.memory``). Point stabbur
at it with ``stabbur chat --mcp memory`` or a ``[[mcp]]`` block.
"""
...
mcp: FastMCP = FastMCP("stabbur-memory")


def _store() -> MemoryStore:
    """Build the store from the current environment (read per call so STABBUR_* is respected)."""
