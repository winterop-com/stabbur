...
* **sandboxed** — every command runs as ``git -C <repo_root> …`` against exactly one directory
  (``STABBUR_GIT_REPO_ROOT``, default the current directory). A ``repo_root`` that isn't a git work
...
* **read-only** — each tool builds a *fixed* argv (no arbitrary subcommand passthrough), so there's
  ...
  ``STABBUR_GIT_ALLOW_WRITE`` (off by default); no mutating tool is registered while it's off, and none
...
Run standalone over stdio: ``stabbur-mcp-git`` (or ``python -m stabbur.mcp_servers.git``). Point stabbur at it with
``stabbur chat --mcp git`` (opt-in; add it deliberately with ``stabbur mcp add git``).
"""
...
class GitSettings(BaseSettings):
    """Config via ``STABBUR_GIT_*`` env: which repo to inspect and whether writes are allowed."""

    model_config = SettingsConfigDict(env_prefix="STABBUR_GIT_", extra="ignore")

    repo_root: Path = Path(".")  # STABBUR_GIT_REPO_ROOT — the only directory git commands run in
    allow_write: bool = False  # STABBUR_GIT_ALLOW_WRITE — reserved gate; no mutating tools ship yet
...
def build_server(settings: GitSettings) -> FastMCP:
    ...
    otherwise.
    """
    server: FastMCP = FastMCP("stabbur-git")
