"""Stabbur MCP server: core utility tools (text, encoding, hashing, JSON, math, stats).

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) so both the
``stabbur-mcp-utils`` console script and ``python -m stabbur.mcp_servers.utils`` resolve here.
"""
