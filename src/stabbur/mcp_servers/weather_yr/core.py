"""Weather lookups against the free met.no (yr.no) APIs — no MCP, no stabbur imports.
...
(override with ``STABBUR_WEATHER_USER_AGENT``) and coordinates are truncated to 4 decimals to aid
their caching.
"""
...
class WeatherSettings(BaseSettings):
    """Config via ``STABBUR_WEATHER_*`` env. The User-Agent identifies this app to met.no/Nominatim."""

    model_config = SettingsConfigDict(env_prefix="STABBUR_WEATHER_", extra="ignore")

    user_agent: str = "stabbur-mcp-weather-yr/0.1 github.com/winterop-com/stabbur"
