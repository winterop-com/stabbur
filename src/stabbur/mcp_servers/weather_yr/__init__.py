"""Stabbur MCP server: weather forecasts from the free met.no (yr.no) APIs.

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) from ``app`` so both the
``stabbur-mcp-weather-yr`` console script and ``python -m stabbur.mcp_servers.weather_yr`` resolve against the
package root.
"""
