...
fixed so there is no arbitrary-fetch surface. See :mod:`stabbur.mcp_servers.weather_yr.core`.

Run standalone over stdio: ``stabbur-mcp-weather-yr`` (or ``python -m stabbur.mcp_servers.weather_yr``). Point
stabbur at it with ``stabbur chat --mcp weather-yr`` or a ``[[mcp]]`` block.
"""
...
from stabbur.mcp_servers.weather_yr.core import Location, WeatherSettings, fetch_forecast, geocode

mcp: FastMCP = FastMCP("stabbur-weather-yr")
