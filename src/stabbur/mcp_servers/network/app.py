...
Run standalone over stdio: ``stabbur-mcp-network`` (or ``python -m stabbur.mcp_servers.network``). Point stabbur at
it with ``stabbur chat --mcp stabbur-mcp-network`` (it's a bundled base server, added by default).
"""
...
mcp: FastMCP = FastMCP("stabbur-network")
