"""Stabbur MCP server: read-only file access (list / read / search) under one configured root.

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) so both the
``stabbur-mcp-files`` console script and ``python -m stabbur.mcp_servers.files`` resolve against the package root.
"""
