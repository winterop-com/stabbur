"""File access under one configured root — no MCP, no stabbur imports.
...
Writes are off unless ``STABBUR_FILES_WRITABLE`` is set. Config via ``STABBUR_FILES_*`` env.
"""
...
class FilesSettings(BaseSettings):
    """Config via ``STABBUR_FILES_*`` env: which root to expose, whether writes are allowed, caps."""

    model_config = SettingsConfigDict(env_prefix="STABBUR_FILES_", extra="ignore")

    root: Path = Path(".")  # STABBUR_FILES_ROOT — the workspace the server exposes
    writable: bool = False  # STABBUR_FILES_WRITABLE — opt in to write_file
...
def write_text(settings: FilesSettings, path: str, content: str) -> int:
    """Write ``content`` to ``path`` (creating parent dirs). Requires ``STABBUR_FILES_WRITABLE``."""
    if not settings.writable:
        raise PermissionError("writes are disabled — set STABBUR_FILES_WRITABLE=1 to allow them")
    target = safe_join(settings.root, path)
