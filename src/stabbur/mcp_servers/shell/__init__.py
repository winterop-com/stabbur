"""Stabbur MCP server: run host shell commands (read-only allowlist by default; opt-in full mode).

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) from ``app`` so both the
``stabbur-mcp-shell`` console script and ``python -m stabbur.mcp_servers.shell`` resolve against the package root.
