...
* **Full mode, opt-in** — set ``STABBUR_SHELL_UNRESTRICTED=1`` to run *arbitrary* commands through a
...
Every run is bounded by a timeout and its output is capped. Not seeded by ``stabbur setup``; add it
deliberately with ``stabbur mcp add shell``.
"""
...
mcp: FastMCP = FastMCP("stabbur-shell")
...
def _unrestricted() -> bool:
    """Whether full (arbitrary-command) mode is enabled via ``STABBUR_SHELL_UNRESTRICTED``."""
    return os.environ.get("STABBUR_SHELL_UNRESTRICTED", "").strip().lower() in ("1", "true", "yes", "on")
...
def _check_allowed(argv: list[str]) -> None:
    ...
    if checker is not None:
        ...
        raise RuntimeError(
            ...
            "show/list forms are allowed. Set STABBUR_SHELL_UNRESTRICTED=1 to run arbitrary commands."
        )
    ...
    if subs is not None:
        ...
        raise RuntimeError(
            ...
            "Set STABBUR_SHELL_UNRESTRICTED=1 to run arbitrary commands."
        )
    raise RuntimeError(
        ...
        "subcommands of git/systemctl/docker/kubectl. Set STABBUR_SHELL_UNRESTRICTED=1 for arbitrary commands."
    )
...
def run(command: str) -> dict[str, Any]:
    ...
    **Read-only by default**: only non-mutating, no-egress commands are allowed (``ping``, ``dig``,
    ...
    ``STABBUR_SHELL_UNRESTRICTED=1`` is set, it instead runs the command through a shell with **no
