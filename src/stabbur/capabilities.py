...
# the ``.stabbur`` sidecar, so it travels with the library; the model files are immutable once pulled,
...
def _cache_path(model: LibraryModel) -> Path:
    ...
    A directory-based model (gguf/mlx/…) keeps the cache in its own ``.stabbur`` sidecar. An Ollama
    model's ``path`` is the manifest *file*, so a ``.stabbur`` dir can't be made under it (the mkdir
