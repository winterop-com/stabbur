"""stabbur's plugin seam (built on pluginkit).

stabbur is the host. Plugins register under the ``stabbur.plugins`` entry-point group (first-party
...
To avoid an import cycle — stabbur bundles some plugin packages, so a plugin must not import
stabbur at runtime — stabbur passes a :class:`PluginContext` into ``commands``. The plugin calls
...
implementations with their own ``pluginkit.Extension("stabbur")`` (matched by project name),
so they depend on pluginkit, never on stabbur.
"""
...
from stabbur.models import McpSetting
...
PROJECT = "stabbur"
ENTRYPOINT_GROUP = "stabbur.plugins"

extension_point = ExtensionPoint(PROJECT)


class PluginContext(Protocol):
    """Host services stabbur injects into a plugin so the plugin needn't import stabbur.
...
class McpServer(BaseModel):
    ...
    command: str  # spawn command, e.g. "stabbur-mcp-datetime"
    description: str = ""
    ...
    workspace root" but nothing can say *which*. Declaring them lets stabbur show the value in force
    and offer an edit, instead of sending the user to hand-edit ``mcp.json``.
    """


class Specs:
    """stabbur's extension points. A plugin implements matching ``@extension`` methods."""

    @staticmethod
    @extension_point
    def commands(context: PluginContext) -> tuple[str, typer.Typer]:
        """Return ``(group_name, app)`` to mount as a top-level stabbur command group."""
    ...
    def mcp_servers() -> list[dict[str, object]]:
        ...
        so a plugin needn't import stabbur. Lets stabbur discover/validate ``--mcp`` targets and
        populate tool pickers instead of hardcoding them.

        ``settings`` is optional and JSON-shaped all the way down — a list of
        ``{"env", "label", "description", "type", "default"}`` dicts (see :class:`stabbur.models.McpSetting`)
        naming the environment variables the server reads. The values stay plain strings/dicts for the
        same reason the entry itself does: a plugin declares what it understands without importing stabbur.
        """
        raise NotImplementedError


def load_plugins() -> PluginManager:
    """Build a PluginManager and discover plugins from the ``stabbur.plugins`` entry-point group.
