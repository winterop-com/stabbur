...
from stabbur.models import ModelEntry, ModelFormat, ModelSource, PullResult
...
def verify_manifest(manifest_path: Path, models_dir: Path, *, deep: bool = False) -> tuple[list[str], int]:
    ...
    blob and compares — the strongest integrity check stabbur can make. Returns ``(issues, n_blobs)``
    (empty issues = intact).
    """
