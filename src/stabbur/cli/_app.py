...
# browse); `audio` owns text-to-speech; `project` owns the ./stabbur.toml assistant
...
config_app = typer.Typer(
    help="Machine-level defaults (~/.config/stabbur/config.toml): library location + default model.",
    no_args_is_help=True,
)
...
def _version(show: bool) -> None:
    ...
    if show:
        from stabbur import __version__  # noqa: PLC0415

        typer.echo(f"stabbur {__version__}")
        raise typer.Exit()
...
class _HostContext:
    """stabbur's ``PluginContext``: hands plugins the model runtime (serve/complete/resolve)."""
    ...
    def run_agent(
        ...
        from stabbur import agent, capabilities  # noqa: PLC0415
        ...
        async def _go() -> str:
            async with mcp_tools.connect(commands) as toolset:
                ...
                async def _recording_call(
                    ...
                    # with stabbur.benchmark.core.RESULT_ARG_KEY (a deliberate copy: the host CLI path
                ...
                return await agent.run(
                    ...
                    vision=vision,
                    # Programmatic eval harness (stabbur-benchmark): intentionally UNGATED. There is no
                    # human to answer a per-action confirmation here, and the write suites are designed
                    # to run their self-cleaning STABBUR_-prefixed lifecycles unattended. The confirmation
...
def _mount_plugins() -> None:
    """Discover ``stabbur.plugins`` and mount each plugin's command group on the CLI."""
    from stabbur import plugins  # noqa: PLC0415 - keep pluginkit off the hot path for `--help`
