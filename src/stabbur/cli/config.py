"""`stabbur config` - read and write the machine config (get/set/list/path)."""
...
from stabbur.cli._app import config_app
...
def _config_field(key: str) -> str:
    """Resolve a `stabbur config` key to its Settings field name, or exit with a clear error."""
...
def config_get(
    ...
    The effective value folds in env vars / project stabbur.toml / .env that outrank the machine
    config, so `$(stabbur config get server)` is what `stabbur chat` would actually use.
    """
...
def config_list() -> None:
    ...
    # The effective values fold in env vars / project stabbur.toml / .env that outrank this file.
    ...
    console.print(f"  server        {settings.chat_server or '[dim](none — stabbur chat loads per call)[/]'}")
...
def config_set(
    ...
    """Set a machine default (persisted to the config file), e.g. `stabbur config set model <name>`."""
