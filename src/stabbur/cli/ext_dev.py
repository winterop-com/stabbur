"""`stabbur ext-dev` - interactive extension test-drive launcher (repo-only dev tool).
...
launch a HEADED Chromium with the extension loaded against a real ``stabbur serve`` (the live-tier
...
from stabbur.cli._common import console
...
def _find_repo_root(start: Path) -> Path | None:
    ...
    ``stabbur ext-dev`` is a source-checkout-only dev tool: it needs the actual extension tree to
...
def ext_dev(
    ...
    """Test-drive the browser extension: headed Chromium + a real `stabbur serve`, until Ctrl+C.

    A repo-only dev tool (needs the extension source): builds the extension fresh, launches a
    headed Chromium with it loaded, and starts the live-tier `stabbur serve` (locked model + DHIS2
    ...
    if repo_root is None:
        console.print(
            "[red]stabbur ext-dev runs from a stabbur source checkout[/] (extension/wxt.config.ts not found)."
        )
        raise typer.Exit(1)
    ...
    env = os.environ.copy()
    if multi:
        env["STABBUR_EXT_DEV_MULTI"] = "1"
    env["STABBUR_EXT_DEV_FLAVOR"] = flavor.value
    ...
    # straight to bun/try.ts, whose handler already tears down the browser + stabbur serve. try.ts owns
