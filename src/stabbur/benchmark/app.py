...
stabbur's ``stabbur benchmark`` command drives suites deterministically by importing ``core``
...
Run standalone over stdio: ``stabbur-benchmark`` (or ``python -m stabbur.benchmark``).
"""
...
mcp: FastMCP = FastMCP("stabbur-benchmark")
