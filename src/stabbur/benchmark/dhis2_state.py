"""Live DHIS2 read-back + STABBUR_ residue sweep for the stateful write benchmark.
...
:func:`stabbur.benchmark.core.score_tool_stateful`). This module holds the concrete DHIS2 client
(:class:`Dhis2StateReader`, a :class:`~stabbur.benchmark.core.StateReader`) plus the between-models
residue sweep that deletes any ``STABBUR_``-prefixed leftovers so a model that abandons a lifecycle
...
def load_profile(name: str | None, profiles_path: Path | None = None) -> Dhis2Profile | None:
    ...
    Environment overrides (win over the file so CI can point elsewhere): ``STABBUR_BENCH_DHIS2_URL``,
    ``STABBUR_BENCH_DHIS2_USER``, ``STABBUR_BENCH_DHIS2_PASSWORD``. Otherwise reads
    ...
    """
    env_url = os.environ.get("STABBUR_BENCH_DHIS2_URL")
    if env_url:
        return Dhis2Profile(
            base_url=env_url,
            username=os.environ.get("STABBUR_BENCH_DHIS2_USER", "admin"),
            password=os.environ.get("STABBUR_BENCH_DHIS2_PASSWORD", "district"),
        )
    if not name:
        return None
...
class Dhis2StateReader:
    """A live DHIS2 read/delete client for verifying and cleaning ``STABBUR_`` write residue.
    ...
    name is not ``STABBUR_``-prefixed, so the sweep can only ever touch benchmark test objects.
    """
    ...
    def sweep(self, resource: str, prefix: str = STABBUR_PREFIX) -> list[str]:
...
def sweep_residue(reader: Dhis2StateReader, resources: list[str], prefix: str = STABBUR_PREFIX) -> dict[str, list[str]]:
    """Sweep ``STABBUR_`` residue across ``resources``; return ``{resource: [deleted uids]}``.
