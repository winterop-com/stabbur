"""Benchmark core: suites and scoring — no MCP, no stabbur deps.
...
We run each candidate in a throwaway Docker container (via :mod:`stabbur_sandbox`) and compare
...
# Re-exported (``import X as X``) so callers can keep importing them from ``core`` — stabbur's
# driver, the MCP app, and tests do.
from stabbur_sandbox import RunResult as RunResult
from stabbur_sandbox import docker_available as docker_available
from stabbur_sandbox import run_code as run_code

Difficulty = Literal["basics", "intermediate", "advanced", "expert"]
ProblemType = Literal["code", "tool"]

#: Reserved args key under which stabbur's recording agent-loop (``stabbur.cli._app.run_agent``)
...
class ExpectAbsent(BaseModel):
    ...
    or ``STABBUR_`` prefix, queried live with ``filter=name:like:<name>``. A stateful ``tool``
...
class Suite(BaseModel):
    ...
    type: ProblemType = "code"
    language: str
    # On a `stabbur benchmark run --all`, only models carrying this library tag are run against
...
class StateReader(Protocol):
    ...
    The one method a scorer needs; :class:`stabbur.benchmark.dhis2_state.Dhis2StateReader` is the
...
def score_tool_stateful(
    ...
    cases = [
        ...
        CaseResult(
            passed=state_passed,
            expected="all STABBUR_ objects absent on the live instance at end",
            actual=state_actual,
            error=residue,
        ),
        CaseResult(
...
def render_leaderboard(records: list[RunRecord]) -> str:
    ...
    if not records:
        return "_No benchmark results yet. Run `stabbur benchmark run <suite> --model <name> --save`._\n"
