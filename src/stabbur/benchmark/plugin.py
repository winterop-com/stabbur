"""stabbur plugin: the ``stabbur benchmark`` command group (a pluginkit extension).

Contributes ``stabbur benchmark list | run | leaderboard``. The run driver serves a model
...
leaderboard. It reaches stabbur's runtime only through the injected
:class:`~stabbur.plugins.PluginContext`, so this package never imports stabbur at runtime.
"""
...
if TYPE_CHECKING:
    ...
    from stabbur.benchmark.dhis2_state import Dhis2StateReader
...
def _make_reader(suite: core.Suite) -> tuple[Dhis2StateReader | None, list[str]]:
    ...
    if not any(p.expect_absent for p in suite.problems):
        return None, []
    try:
        from stabbur.benchmark import dhis2_state  # noqa: PLC0415 - optional; only for write suites
...
def _sweep(console: Console, reader: Dhis2StateReader | None, resources: list[str], when: str, model: str) -> None:
    """Best-effort STABBUR_ residue sweep around a model's write suite (``when`` = before/after)."""
    if reader is None or not resources:
        return
    try:
        from stabbur.benchmark import dhis2_state  # noqa: PLC0415
        ...
        if total:
            ...
            console.print(f"  [dim]swept {total} STABBUR_ residue {when} {model} ({detail})[/]")
...
def _build_app(context: PluginContext) -> typer.Typer:
    ...
    @bench.command("run")
    def run(
        suite: Annotated[str, typer.Argument(help="Suite name (see `stabbur benchmark list`).")],
        model: Annotated[
        ...
        for mdl in models:
            ...
            # Clear any STABBUR_ residue a prior model abandoned so it can't compound into this run.
...
class BenchmarkPlugin:
    """stabbur plugin object: contributes the ``benchmark`` command group."""

    @extension
    def commands(self, context: PluginContext) -> tuple[str, typer.Typer]:
        """Mount ``stabbur benchmark`` (``list`` / ``run`` / ``leaderboard``)."""
        return "benchmark", _build_app(context)

    # Note: benchmark deliberately does NOT advertise itself via `mcp_servers`. It's a
    # dev/benchmarking tool (driven by `stabbur benchmark` + its own MCP app), not an
