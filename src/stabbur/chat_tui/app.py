...
from stabbur import agent, attach, capabilities, project, transcript
...
class RemoteEndpoint(BaseModel):
    ...
    Built by ``stabbur chat --server``: the TUI chats against ``base`` but must not stop the
    ...
    n_ctx: int | None = None  # context window reported by stabbur serve's /api/status, if it answered


class ChatApp(App[None]):
    """The stabbur chat TUI: transcript + input + pinned status footer."""
    ...
    def __init__(
        ...
        *,
        ...
        self._reason_prog: dict[int, bool] = {}  # per reasoning box: the collapsed state stabbur set itself
    ...
    def _remember_reasoning_collapse(self, event: Collapsible.Toggled) -> None:
        ...
        ``@on(Collapsible.Toggled)`` catches both the Expanded and Collapsed subclasses. stabbur toggles
        the box itself (on mount, and the auto-collapse in ``finalize_reasoning``); those are ignored
        by comparing against the last state stabbur set for that box, so only a real user click sticks.
        """
    ...
    def _intro(self) -> RenderableType:
        ...
        try:
            ver = _pkg_version("stabbur")
        except Exception:  # noqa: BLE001 - version is cosmetic; never block the intro on it
            ver = ""

        title = Text()
        title.append("♥ ", style="#f43f5e")  # heartbeat — stabbur is 鼓動, "heartbeat/pulse"
        title.append("stabbur", style="bold #f43f5e")
        if ver:
    ...
    def action_show_mcp(self) -> None:
        ...
        if not servers:
            self._post(Text("No MCP tools loaded — add servers with `stabbur mcp add <name>`.", style="grey50"))
            return
    ...
    async def _generate(self, raw_text: str) -> None:
        ...
        try:
            ...
            self._reason_prog[id(reasoning_box)] = self._reason_collapsed_pref  # stabbur set this state
