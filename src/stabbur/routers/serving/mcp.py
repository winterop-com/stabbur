...
def list_mcp_servers() -> list[BundledMcp]:
    """Every first-party MCP server stabbur ships, with whether it is currently enabled.
...
async def set_mcp_server(name: str, body: McpUpdateRequest, request: Request) -> McpUpdateResult:
    ...
    require editing the project's own ``.mcp.json`` (committed and portable — stabbur never rewrites it from
