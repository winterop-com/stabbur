...
stabbur is multi-process in practice: the CLI and ``stabbur serve`` are expected to run at the same
...
A single advisory file lock per library — ``<root>/.stabbur/lock`` — serializes those short
critical sections across processes. It is **best-effort**: a platform without ``fcntl`` or a
filesystem that rejects the lock degrades to a no-op rather than blocking work (stabbur already
...
def library_lock(root: Path) -> Generator[None]:
    ...
    Wrap a read-modify-write (tags) or a destructive op (remove/migrate) so a second stabbur
    ...
    """
    lock_path = root / ".stabbur" / _LOCK_FILENAME
    try:
