...
# The documented example every project ships beside its manifest. One string, one writer: the
# repo's own stabbur.example.toml is generated from this too (a test keeps them identical), so
...
def render_example_manifest() -> str:
    """The fully documented ``stabbur.example.toml`` shipped beside every scaffolded project.

    A manifest with four keys in it teaches four keys. The example is where the rest live —
    remote backends, the serving and runtime knobs, where tools actually go — as a file an editor
    highlights (hence ``.example.toml``, not ``.toml.example``) and nothing reads.
    """
