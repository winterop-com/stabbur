"""Project templates for ``stabbur project new --template <name>``.
...
from stabbur.models import ProjectTemplate
...
Copy these into `uv run stabbur chat` (or the web UI). The assistant is bound to a DHIS2-capable
...
DHIS2 (localhost:8080), not a shared demo. Prefix throwaway objects with `STABBUR_` so they're
...
## Create
- Create a NUMBER aggregate data element named 'STABBUR_ANC_TEST' (short name 'STABBUR_ANC') and
  tell me its UID.
- Create an indicator group named 'STABBUR_TEST_GROUP'.

## Update / rename
- Rename the data element 'STABBUR_ANC_TEST' to 'STABBUR_ANC_TEST_v2'.
- Give 'STABBUR_ANC_TEST_v2' the description 'created by the stabbur write assistant'.

## Delete (confirm first)
- Delete the data element 'STABBUR_ANC_TEST_v2'. Show me it's gone afterwards.
"""
...
- Base64-encode the string "stabbur rocks", then decode it back.
...
- Search the project for the string "stabbur.toml" and show where it appears.

The files server is read-only by default (rooted at the project directory). Set
`STABBUR_FILES_WRITABLE=1` in `.env` to allow writes, and `STABBUR_FILES_ROOT` to change the root.
"""
...
the web UI (`uv run stabbur serve --ui`) and enable Listen, or use `uv run stabbur chat`.

- Introduce yourself in one sentence.
- Tell me a very short joke.
- Read this back to me: "the quick brown fox".

Pick a different Kokoro voice with `chat_voice` in `stabbur.toml` (see `uv run stabbur voice voices`).
"""
...
hear replies spoken back (Kokoro). Open the web Voice surface with `uv run stabbur serve --ui`.
    ...
    *,
    ...
    def _mode(auth: str, secret_env: str, extra_secret_env: str | None = None) -> dict[str, Any]:
        mode: dict[str, Any] = {
            ...
            # --yes: d2w >= 1.3 confirms interactively; stabbur runs bind commands with stdin closed,
...
TEMPLATES: dict[str, ProjectTemplate] = {
    ...
    "research": ProjectTemplate(
        ...
        mcp=[
            ("search", "stabbur-mcp-search"),  # bundled with stabbur -> no extra pin
    ...
    "dhis2": ProjectTemplate(
        ...
        next_steps=(
            ...
            "  uv sync && uv run stabbur project show      # confirm the model + dhis2 tools are wired\n"
            "  uv run stabbur serve --ui                   # or: uv run stabbur chat\n"
            "Point it at your own instance: edit .dhis2/profiles.toml (or `d2w profile add … --local`) "
            "and DHIS2_PROFILE in stabbur.toml; drop DHIS2_MCP_READONLY=1 to allow writes."
        ),
    ),
    "dhis2-write": ProjectTemplate(
        ...
        next_steps=(
            ...
            "  uv sync && uv run stabbur project show\n"
            "  uv run stabbur serve --ui                   # or: uv run stabbur chat\n"
            "The bridge runs read-write (no DHIS2_MCP_READONLY). Prefix test objects with STABBUR_ so "
    ...
    "dhis2-multi": ProjectTemplate(
        ...
        next_steps=(
            ...
            "  mkdir -p .dhis2 && cp examples/dhis2-profiles.toml .dhis2/profiles.toml   # demo: admin/district\n"
            "  uv sync && uv run stabbur project show      # confirm both targets + their bridges are wired\n"
            "  uv run stabbur serve --ui                   # panel routes tools by the active tab\n"
            "  uv run stabbur chat --target play41         # in the CLI, pick a target (default: play42)\n"
            "Point at your own instances: edit .dhis2/profiles.toml, the two base_urls in stabbur.toml, and\n"
    ...
    "mcp-datetime": ProjectTemplate(
        ...
        mcp=[("datetime", "stabbur-mcp-datetime")],
        files={"examples/prompts.md": _DATETIME_PROMPTS_MD},
        next_steps="  uv sync && uv run stabbur chat",
    ),
    ...
    "mcp-web": ProjectTemplate(
        ...
        next_steps=(
            ...
            "  uv sync && uv run playwright install chromium && uv run stabbur chat"
        ),
    ),
    ...
    "mcp-weather": ProjectTemplate(
        ...
        mcp=[("weather", "stabbur-mcp-weather-yr")],
    ...
    "mcp-files": ProjectTemplate(
        ...
        mcp=[("files", "stabbur-mcp-files")],
        files={"examples/prompts.md": _FILES_PROMPTS_MD},
        next_steps=(
            "Read-only by default, rooted at the project. Set STABBUR_FILES_WRITABLE=1 / STABBUR_FILES_ROOT in .env.\n"
            "  uv sync && uv run stabbur chat"
        ),
    ),
    ...
    "voice-kokoro": ProjectTemplate(
        ...
        next_steps=(
            ...
            "  uv sync && uv run stabbur serve --ui        # enable Listen, or: uv run stabbur chat"
        ),
    ),
    "voice-whisper": ProjectTemplate(
        ...
        next_steps=(
            ...
            "  uv sync && uv run stabbur serve --ui        # use the Voice surface"
        ),
    ),
}
