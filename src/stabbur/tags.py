...
live *inside the library itself* (``<library_root>/.stabbur/tags.json``, a
...
from stabbur import fsatomic, locking
...
def _path(library_root: Path) -> Path:
    return library_root / ".stabbur" / _FILENAME
...
def _registry_path(library_root: Path) -> Path:
    return library_root / ".stabbur" / _REGISTRY_FILENAME
