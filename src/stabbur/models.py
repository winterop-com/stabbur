...
class ModelSource(StrEnum):
    ...
    ollama = "ollama"
    lmstudio = "lmstudio"
    voice = "voice"  # stabbur's registry of TTS/STT models (by short id, e.g. "kokoro"); lands in <root>/voice/
...
class HubModel(BaseModel):
    """A model found on the Hugging Face Hub (a `stabbur pull` candidate)."""

    id: str
    downloads: int = 0
    likes: int = 0
    size_bytes: int = 0  # approx bytes stabbur would pull (preferred quant); 0 = unknown


class CuratedModel(BaseModel):
    """A curated starter model offered by `stabbur init`."""
...
class CuratedMcp(BaseModel):
    """A curated MCP tool server offered by `stabbur mcp list` / added by `stabbur mcp add`."""

    model_config = ConfigDict(frozen=True)

    name: str  # tool namespace + the .mcp.json server key, e.g. "dhis2"
    command: str  # the command to run it (added to .mcp.json by `stabbur mcp add`)
    description: str
...
class McpSetting(BaseModel):
    ...
    A server's behaviour hangs on its env (``STABBUR_FILES_ROOT`` decides the *only* directory the
    ...
    to change it. A server declares these alongside its command (see :meth:`stabbur.plugins.Specs.mcp_servers`)
    and :func:`stabbur.mcp_catalog.bundled` fills in :attr:`effective`.
    ...
    env: str  # the variable itself, e.g. "STABBUR_FILES_ROOT"
    ...
    path default resolved against the directory ``stabbur serve`` runs in, since "``.``" is precisely
    the answer that leaves a user guessing which directory the assistant can browse."""


class BundledMcp(BaseModel):
    """One first-party MCP server stabbur ships, plus whether it is currently switched on.

    The unit the Tools UI renders: stabbur bundles a dozen ``stabbur-mcp-*`` servers, but until one is
    ...
    client show "here is everything stabbur can do, these are on" instead of an empty list.
    """

    model_config = ConfigDict(frozen=True)

    name: str  # tool namespace + the mcp.json server key, e.g. "datetime"
    command: str  # the console script that runs it, e.g. "stabbur-mcp-datetime"
    description: str = ""
    enabled: bool = False  # in the resolved global+project set, i.e. stabbur spawns it
...
class ProjectTemplate(BaseModel):
    """A named starter for `stabbur project new --template <name>`.
    ...
    extras: list[str] = Field(default_factory=list)  # stabbur extras the uv project needs, e.g. ["voice"], ["web"]
    ...
    # dict here so models.py needn't import stabbur.project.AssistantInfo — avoids an import cycle.
