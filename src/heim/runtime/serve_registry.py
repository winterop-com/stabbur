"""Discovery of running ``heim serve`` processes, so ``heim chat`` can reuse a loaded model.

Each ``heim serve`` writes a small record (its base URL + the model it locked + its pid) under the
machine runtime dir; ``heim chat``, given no explicit ``--server``, looks here for a **live** server
...
Only **unauthenticated** serves register — a loopback ``heim serve`` with no bearer token. A serve
that needs auth (non-loopback, or an explicit ``HEIM_AUTH_TOKEN``) is left out, since ``heim chat``
...
class ServeRecord(BaseModel):
    """A running ``heim serve``'s discovery record."""
...
def register(base_url: str, model: str | None) -> Path | None:
    """Record this ``heim serve`` (its base URL + locked model) so ``heim chat`` can find it.
...
def unregister() -> None:
    """Remove this process's serve record (called when ``heim serve`` stops)."""
    try:
...
def discover(model: str) -> ServeRecord | None:
    ...
    for record_file in directory.glob("*.json"):
        ...
        # older heim) or whose live cmdline no longer matches is stale; sweep it like a dead pid.
