...
class LibraryNotConfigured(RuntimeError):
    """No library location is configured — heim can't do anything useful without one.

    The shared/default library must be set explicitly (``HEIM_LIBRARY_ROOT``) rather than
    silently falling back to a CWD-relative ``./data`` (which is meaningless for a globally
    installed CLI). A project (``heim.toml``) that lists its own ``libraries`` counts as
    configured. Carries a ready-to-print, actionable message.
    """

    HINT = (
        "No library configured. Point heim at your model library — set HEIM_LIBRARY_ROOT:\n"
        "  export HEIM_LIBRARY_ROOT=/path/to/your/library   # an external drive is ideal\n"
        "or run `heim project init` to scaffold a project with its own library."
    )
...
def roots(settings: Settings | None = None) -> list[Path]:
    ...
    A project (``heim.toml``) may list ``libraries`` — paths relative to the project
    ...
    # @shared resolves to the machine default library (HEIM_LIBRARY_ROOT), which must be set
...
def default_root(settings: Settings | None = None) -> Path:
    """The machine's default library (``HEIM_LIBRARY_ROOT`` / ``@shared``), or raise.
