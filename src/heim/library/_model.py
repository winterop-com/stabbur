...
if TYPE_CHECKING:
    from heim.voice.registry import VoiceModel
...
# (shared) library" — so a committed heim.toml stays portable (no hard-coded path).
...
class LibraryModel(BaseModel):
    ...
    """True if this lives in the Ollama store — runnable only via Ollama, not heim."""
...
def _voice_spec(name: str) -> "VoiceModel | None":
    ...
    from heim.voice import registry  # noqa: PLC0415 - avoid importing voice on every library import
