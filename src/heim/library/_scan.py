...
from heim.library._model import (
    LibraryModel,
    ModelRef,
    _classify_dir,
    _clean_name,
...
from heim.library._roots import roots
...
def _scan_dirs(base: Path) -> list[LibraryModel]:
    ...
    for pattern in ("*.gguf", "*.safetensors"):
        for weights in base.rglob(pattern):
            ...
            # Skip anything under a dot-dir: the redirected HF cache (.cache), heim sidecars
            # (.heim), and interrupted-pull staging (.heim-stage-*) are not library models.
