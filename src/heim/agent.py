...
The model may emit ``tool_call``s; heim executes each via the MCP toolset, feeds
...
from heim.tools import MCPToolset, ToolResult
...
async def run(
    ...
    ``None`` (default) reads ``HEIM_TOOL_TIMEOUT`` (120s; set 0 to disable the bound).
    ...
    if tool_timeout is None:
        from heim.config import get_settings  # noqa: PLC0415 - lazy to keep agent import light
