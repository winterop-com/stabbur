"""Atomic, durable file writes for heim's small config/index files.

One implementation of the write-temp-then-rename pattern used by every small file heim can't
afford to corrupt (tags, machine config, ``mcp.json``): stage into a **per-write unique** temp
file (so a concurrent CLI + ``heim serve`` writer can't clobber each other's staging), fsync it,
