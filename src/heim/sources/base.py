...
# Not model content: huggingface_hub's per-download bookkeeping (`.cache/huggingface/`), heim's
# own sidecar (`.heim/`), and macOS AppleDouble noise (`._*`) on exFAT. Excluding them keeps
# recorded sizes accurate — otherwise the transient `.cache/` blobs+metadata inflate the count.
_STATS_SKIP_DIRS = frozenset({".cache", ".heim"})


def _real_file_sizes(path: Path) -> dict[str, int]:
    """``{relative_posix_path: size}`` of a tree's real files (skipping ``.cache``/``.heim``/``._``)."""
    out: dict[str, int] = {}
...
def dir_stats(path: Path) -> tuple[int, int]:
    ...
    Skips huggingface_hub's ``.cache/`` bookkeeping, heim's ``.heim/`` sidecar, and macOS
...
def copy_tree(src: Path, dest: Path) -> tuple[int, int]:
    ...
    tmp = Path(tempfile.mkdtemp(prefix=".heim-stage-", dir=dest.parent))
    try:
