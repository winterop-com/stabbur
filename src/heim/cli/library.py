"""`heim library` - list, pull, remove, tag, verify, install, search, and sync the model library."""
...
from heim.cli._app import library_app
...
from heim.models import ModelFormat, ModelSource, _human_size
from heim.sources import huggingface as hf
...
def list_models(
    ...
    Scans the libraries in scope (a project's ``libraries`` in ``heim.toml``, else
    the default ``HEIM_LIBRARY_ROOT``). To browse models in your app caches that you
    *could* pull, use ``heim library sources``. Pass ``--details`` (``-d``) for a
    ...
    if not models and not voices:
        ...
        console.print("[dim]Pull one with[/] heim library pull [dim]· browse with[/] heim library sources")
        return
    ...
    if voices:  # the Voice category (TTS/STT) — a peer of the format groups
        ...
        console.print(vt)
        console.print("[dim]Voice-specific ops:[/] heim voice ls / import")
...
def remove_model(
    ...
    Resolves like ``heim chat`` (use ``--format`` to disambiguate a model kept in
    ...
    if not copies:
        typer.secho(f"No library model matches {name!r} (see `heim library ls`).", fg=typer.colors.RED, err=True)
        raise typer.Exit(1)
...
def migrate(
    ...
    Older Hugging Face pulls landed in ``huggingface/<repo>``; heim now stores by format. This
...
def install(
    ...
    ``mlx_vlm.server`` run a loose MLX copy in place (heim serves MLX this way), so there's
    no ``--to mlx_lm``.
    """
...
def uninstall(
    ...
    """Remove a model from a runtime (undo `heim library install`). The library copy is kept.

    ``--from lmstudio`` removes only heim's symlink (never a real LM Studio download); ``--from
...
def installed() -> None:
    ...
    and whether LM Studio has a heim symlink to it. Undo any of these with `heim library uninstall`.
    """
    ...
    if not lines:
        console.print(
            ...
            "[dim]Feed one to a runtime with[/] heim library install <model> --to ollama|lmstudio"
        )
        return
...
# Display-format column order for `heim library formats`. Ollama's content-addressed store is
# its own column (a distinct consumer copy), not folded into gguf. gguf/mlx are the ready-to-run
# quants heim serves; safetensors is the convert/fine-tune source (2-4x a quant, not runnable).
...
def formats() -> None:
    ...
    that copy's size, and a NOTE flagging the policy cases (see ``heim library ls`` for the full
    ...
    if not by_name:
        ...
        console.print("[dim]Pull one with[/] heim library pull [dim]· see[/] heim library ls")
        return
    ...
    for name in sorted(by_name):
        ...
        if "safetensors" in fmts and has_quant:
            ...
            note = (
                ...
                f"[dim]· heim library rm {name} --format safetensors[/]"
            )
...
def cards_backfill(
    ...
    """Backfill missing Hugging Face model cards into library models' ``.heim/`` sidecars.
...
def verify_library(
    ...
    Verifies each model's total size, file count, and card against its ``.heim/metadata.json``
...
def manifest(
    ...
    """Export your library as a re-pullable want list (TOML) — the input for `heim library sync`.

    Reads each model's recorded source (its ``.heim/`` sidecar, or inferred for older pulls) and
    emits a portable ``[[model]]`` list. Keep the file wherever you like — commit it, or copy it to
    a new drive — then ``heim library sync <file>`` re-downloads everything in it that's missing. No
...
@library_app.command()
def sync(
    wantfile: Annotated[Path, typer.Argument(help="A want list (TOML) produced by `heim library manifest`.")],
    dry_run: Annotated[
...
def tag_style(
    ...
    Assignments stay plain name references (``heim library tag``); this stores the tag's *style*
...
def pull(
    ...
    ``heim library pull voice kokoro``) into ``<root>/voice/`` — downloading it if it
    isn't already in the Hugging Face cache. This is the project-aware way to add a
    voice model (``heim voice import`` is the older cache-only alias).
    """
...
def sources(
    ...
    local models onto the drive with ``heim library pull --move`` to free local disk.
    ...
    console.print("[dim]Caches on this machine, not your library — see[/] heim library ls [dim]for your library.[/]\n")
...
def search(
    ...
    console.print(
        ...
        "Pull one with[/] heim library pull huggingface <model>"
    )
