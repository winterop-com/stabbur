...
from heim.cli._app import voice_app
...
def voices() -> None:
    ...
    if not kokoro.available():
        typer.secho("Kokoro TTS is unavailable — reinstall heim (`uv sync`).", fg=typer.colors.YELLOW)
        raise typer.Exit(1)
    ...
    console.print(table)
    console.print(f'\n[dim]{len(kokoro.voices())} voices — use with[/] heim voice speak --voice <id> "…"')
...
def speak(
    ...
    voice: Annotated[
        str | None,
        typer.Option("--voice", "-v", help="Kokoro voice id (e.g. af_heart; see `heim voice voices`)."),
    ] = None,
    model: Annotated[
        str | None,
...
def _synth_kokoro(text: str, voice: str, *, speed: float = 1.0) -> bytes:
    ...
    if not kokoro.available():
        typer.secho("Kokoro TTS is unavailable — reinstall heim (`uv sync`).", fg=typer.colors.RED, err=True)
        raise typer.Exit(1)
...
def _finish_speak(data: bytes, fmt: str, output: Path | None, play: bool) -> None:
    ...
    path to the OS audio player — so repeated ``heim voice speak`` calls don't litter ``/tmp``.
    """
...
@voice_app.command("ls", hidden=True)  # alias, matching `heim library ls`
def voice_list() -> None:
    ...
    ``heim library ls`` — so a model on the shared drive shows as ``library`` here too.
    """
    from heim import voice  # noqa: PLC0415
    ...
    console.print(table)
    console.print(
        "[dim]Add one to the project library:[/] heim library pull voice <id>  [dim](downloads if needed).[/]"
    )
...
def voice_import(
    ...
    """Import voice models into a library — a project-aware alias for ``heim library pull voice``.
    ...
    """
    if all_ and models:  # like `heim library pull`, reject the contradictory combination
    ...
    if all_:
        ...
        return
    if not models:
        console.print("[red]Give a model id or --all[/] (see [bold]heim voice list[/]).")
        raise typer.Exit(1)
    failed = False
    for vid in models:
    ...
    console.print("[dim]Same thing, project-aware:[/] heim library pull voice <id>")
    if failed:
        raise typer.Exit(1)
