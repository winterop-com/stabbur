"""`heim mcp` - browse the MCP catalog and add/remove servers in .mcp.json."""
...
from heim.cli._app import mcp_app
...
@mcp_app.command("ls", hidden=True)  # `ls` alias, mirroring `heim library ls`
def mcp_list() -> None:
    ...
    if servers:
        plug = Table(
            box=box.SIMPLE,
            header_style="bold",
            title="[bold]Installed plugins[/] [dim]— first-party, heim-controlled (recommended)[/]",
            title_justify="left",
        )
    ...
    console.print("[dim]Add one to this project's .mcp.json (plugins or catalog):[/] heim mcp add <name>")


@mcp_app.command("add")
def mcp_add(
    name: Annotated[str, typer.Argument(help="An installed plugin or external-catalog name (see `heim mcp list`).")],
    to_global: Annotated[
        bool,
        typer.Option(
            "--global", "-g", help="Add to the machine-global ~/.config/heim/mcp.json instead of this project."
        ),
    ] = False,
) -> None:
    ...
    ``~/.config/heim/mcp.json`` with ``--global`` (tools every free-play chat gets). Resolves
    ...
    # First-party plugins win a name clash — heim controls them, so prefer them over external.
    ...
    if server is not None:
        ...
        if chosen is None:
            console.print(f"[red]Unknown MCP {name!r}.[/] See [bold]heim mcp list[/] for the catalog.")
            raise typer.Exit(1)
    ...
    console.print(f"[dim]Check it:[/] {'heim doctor' if to_global else 'heim project show'}")


@mcp_app.command("remove")
@mcp_app.command("rm", hidden=True)
def mcp_remove(
    name: Annotated[str, typer.Argument(help="The server name to remove (see `heim project show`).")],
...
def mcp_tools() -> None:
    ...
    Connects to the resolved servers (``~/.config/heim/mcp.json`` + ``./.mcp.json``) and lists the
    actual tools each one exposes — unlike ``heim mcp list``, which shows the servers/catalog. This
    is "what can my assistant actually do" in one place.
    """
    from heim.cli.project import _connect_project_tools  # noqa: PLC0415

    servers = mcpservers.resolve()
    if not servers:
        console.print("[dim]No MCP servers configured.[/] Add one with [bold]heim mcp add[/] (or [bold]--global[/]).")
        return
