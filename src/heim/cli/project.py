"""`heim project` - scaffold (init/new) and inspect (show) a project assistant."""
...
from heim.cli._app import project_app
...
from heim.models import ModelSource, ProjectTemplate
from heim.project import scaffold
...
def _pick_tools_interactive() -> list[tuple[str, str]]:
    ...
    if not servers:
        console.print("\n[bold]2. Tools[/] [dim]— no MCP plugins installed; skipping (add later in heim.toml)[/]")
        return []
...
def _provision_model(target: Path, model: str, local: bool) -> tuple[bool, list[library_ops.LibraryModel]]:
    ...
    use_local = local or not shared  # --local/--copy forces it; no HEIM_LIBRARY_ROOT falls back to it
    if not use_local:
        if existing:
    ...
    if existing:
        ...
        console.print(f"\n[yellow]No HEIM_LIBRARY_ROOT set[/] — downloading [bold]{model}[/] into {_LOCAL_LIBRARY}/.")
...
def _scaffold_project(
    ...
    a spoken-reply voice, then writes ``target/heim.toml``. By default it uses your machine
    library (HEIM_LIBRARY_ROOT), pulling a missing model into it. ``local`` (``--local``/
    ...
    HEIM_LIBRARY_ROOT set it falls back to that project-local library so you can start fresh.

    ``uv`` (default on) also makes the project a **self-contained uv project**: a
    ``pyproject.toml`` pinning ``heim`` + its MCP servers, so ``uv run heim serve`` uses the
    project's own environment instead of a global heim and runtime ``uvx`` fetches.
    """
    proj = target / "heim.toml"
...
def init(
    ...
    A project is a purpose-built assistant: `heim serve`/`chat` here bind to its model
    (like --model) with its tools + prompt; outside a project it's free-play. Use
    `heim project new <dir>` to scaffold into a fresh directory instead. `--template dhis2`
    ...
    console.print(f"[dim]Next:[/] {'uv sync && uv run heim serve --ui' if uv else 'heim serve --ui'}")
...
def new(
    ...
    console.print(f"[dim]Next:[/] cd {name} && {'uv sync && uv run heim serve --ui' if uv else 'heim serve --ui'}")
...
def project_(
    ...
    """Show the active project (heim.toml): model + card, system prompt, and live tools.

    A project is a reproducible assistant — `heim chat` and `heim serve --ui` here
    ...
    proj = project.load()
    if proj is None:
        console.print("[yellow]No heim.toml here.[/] Run [bold]heim project init[/] to scaffold a project.")
        raise typer.Exit(1)

    console.print("\n[bold]Project[/] [dim](heim.toml)[/]\n")
    ...
    if proj.model:
        ...
        if matches:
            ...
            console.print(f"[bold]Model[/]  {proj.model}  [yellow]not in library — run heim project init[/]\n")
    else:
    ...
    if not servers:
        ...
        for m in servers:
            ...
            if m.name in failed:  # this one couldn't start; the others still work
                ...
                if m.command.startswith("heim-mcp-web"):
                    console.print(
