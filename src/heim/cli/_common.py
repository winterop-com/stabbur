...
from heim.models import CuratedModel, ModelFormat, ModelSource
...
# The MCP catalog (curated + optional first-party servers) lives in `heim.mcp_catalog` so the
...
# Curated starter models for `heim project init` (verified GGUF repos; small but capable
...
#: The project-local library directory that `heim project init` scaffolds (owned by heim.scaffold).
_LOCAL_LIBRARY = scaffold.LOCAL_LIBRARY


def _to_mcp_server(name: str, command: str) -> mcpservers.McpServer:
    """Turn a CLI ``(name, command)`` pair into a standard :class:`heim.mcpservers.McpServer`.
...
def _cli_mcp_spec(value: str) -> tuple[str | None, list[str], dict[str, str]]:
    """Resolve a ``--mcp`` value to the ``(name, argv, env)`` spec :func:`heim.tools.connect` wants.
...
def _pull_voice_all(root: Path | None, move: bool) -> None:
    ...
    Mirrors the old ``heim voice import --all`` (no mass downloads): only cached models are
    pulled. Fetch a not-yet-downloaded one by name, e.g. ``heim library pull voice kokoro``.
    """
    from heim.voice import importer as voice_importer  # noqa: PLC0415

    target = root or library_ops.default_root()
    ids = voice_importer.cached_voice_ids(target)
    if not ids:
        console.print("No cached voice models to import. Pull one by name, e.g. heim library pull voice kokoro.")
        return
    failed = 0
    for vid in ids:
        try:
...
_ForceOpt = Annotated[bool, typer.Option("--force", help="Overwrite an existing heim.toml.")]
_LocalOpt = Annotated[
    bool,
...
_UvOpt = Annotated[
    bool,
    typer.Option("--uv/--no-uv", help="Make it a self-contained uv project (pyproject.toml pinning heim + tools)."),
]
...
def _not_chat_msg(name: str, model_format: ModelFormat) -> str:
    return f"[red]{name!r} is not a chat model[/] ({model_format.value}) — heim runs generative LLMs only."
...
def _resolve_library_model(name: str, model_format: ModelFormat | None) -> library_ops.LibraryModel:
    ...
    if not matches:
        ...
        if hit and not hit.generative:
            ...
            console.print(
                ...
                f"heim library pull {hit.source.value} {hit.name}"
            )
        else:
    ...
    if model.is_ollama:
        ...
        console.print(f"[red]{model.name!r} is an Ollama model[/] — heim runs GGUF/MLX, not Ollama's format.")
