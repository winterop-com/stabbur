"""OS-aware helpers so heim runs cleanly on both macOS and Linux.

heim serves on POSIX only (macOS + Linux; see :mod:`heim.supervisor`). The two
...
def package_hint(package: str) -> str:
    ...
    Covers the CLI tools heim shells out to that ship in every mainstream package
...
def install_hints() -> dict[str, str]:
    """Install hints for the runtime binaries heim spawns, keyed by executable name.

    Tailored to the running OS so ``heim doctor`` and runtime errors never point a
