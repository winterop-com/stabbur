...
# the ``.heim`` sidecar, so it travels with the library; the model files are immutable once pulled,
...
def _cache_path(model: LibraryModel) -> Path:
    ...
    A directory-based model (gguf/mlx/…) keeps the cache in its own ``.heim`` sidecar. An Ollama
    model's ``path`` is the manifest *file*, so a ``.heim`` dir can't be made under it (the mkdir
    ...
    """
    from heim.cards import SIDECAR_DIR  # noqa: PLC0415 - avoid an import cycle at module load

    if model.is_ollama:
        # Mirror heim.sources.ollama._safe_name locally to avoid importing the source here.
...
def capabilities(model: LibraryModel) -> ModelCapabilities:
    ...
    Reads a cached result from the ``.heim`` sidecar when present (so a library scan doesn't
