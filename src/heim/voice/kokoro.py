...
no reference audio. It's a base dependency (the always-available in-chat voice); heim
...
This is the multi-voice engine that complements :mod:`heim.tts` (single-voice
...
def _assets_dir() -> Path:
    ...
    """
    from heim import library  # noqa: PLC0415 - lazy to avoid an import cycle
...
def synthesize(text: str, voice: str, out_path: Path | None = None) -> Path:
    ...
    """
    if not available():
        raise RuntimeError("Kokoro TTS is unavailable — reinstall heim's dependencies (`uv sync`).")
