...
heim is multi-process in practice: the CLI and ``heim serve`` are expected to run at the same
...
A single advisory file lock per library — ``<root>/.heim/lock`` — serializes those short
critical sections across processes. It is **best-effort**: a platform without ``fcntl`` or a
filesystem that rejects the lock degrades to a no-op rather than blocking work (heim already
...
def library_lock(root: Path) -> Generator[None]:
    ...
    Wrap a read-modify-write (tags) or a destructive op (remove/migrate) so a second heim
    ...
    """
    lock_path = root / ".heim" / _LOCK_FILENAME
    try:
