"""Project scaffolding — the pure logic behind ``heim project new`` / ``init`` (A7).
...
#: The project-local library directory ``heim project init`` scaffolds (holds this project's models).
LOCAL_LIBRARY = "library"


def heim_repo_root() -> Path:
    """The heim source checkout (repo root), for pinning heim in a project's pyproject."""
...
def pip_deps_from_mcp(mcp: list[tuple[str, str]]) -> list[str]:
    ...
    (``bunx``/``npx``) and heim's own bundled ``heim-mcp-*`` (they ship with the ``heim`` dep).
    """
    deps: set[str] = set()
...
def render_pyproject(name: str, mcp: list[tuple[str, str]], mlx: bool, extras: list[str] | None = None) -> str:
    ...
    Pins ``heim`` (a local path source until heim is on PyPI) plus any pip-installable MCP
    servers, so ``uv run heim serve`` uses this project's own environment — no global heim,
    no runtime ``uvx`` fetches. ``extras`` are heim extras the project needs (e.g. ``voice``,
    ``web``); ``mlx`` adds the MLX runtime extra when the bound model is MLX.
    """
    pkg_name = re.sub(r"[^a-z0-9._-]+", "-", name.lower()).strip("-._") or "heim-project"
    all_extras: set[str] = set(extras or [])
    if mlx:
        all_extras.add("mlx")
    extras_spec = f"[{','.join(sorted(all_extras))}]" if all_extras else ""
    deps = [f"heim{extras_spec}", *pip_deps_from_mcp(mcp)]
    ...
    return (
        ...
        'version = "0.0.0"\n'
        'description = "A heim assistant project."\n'
        'requires-python = ">=3.13"\n'
        "dependencies = [\n"
        f"{dep_lines}"
        "]\n\n"
        "[tool.uv.sources]\n"
        "# heim is not yet on PyPI; pin the local checkout (editable). This line is\n"
        "# machine-specific -- replace it with a version once heim publishes.\n"
        f"heim = {{ path = {heim_root}, editable = true }}\n"
    )


_README = """\
# {name} -- a heim assistant

A self-contained [heim](https://github.com/winterop-com/heim) project: its own uv
environment, model library, and assistant definition (`heim.toml`).

## Run

```bash
uv sync                     # build the project's environment (heim + its MCP servers)
uv run heim serve --ui      # browser UI
uv run heim chat            # terminal chat
```

The assistant (model, system prompt, tools) is defined in `heim.toml`. The model lives in
`{library}/` (not committed). Machine secrets go in `.env` (not committed).
"""

_GITIGNORE = f"""\
# heim project — the local model library holds large weights; don't commit them.
/{LOCAL_LIBRARY}/
