...
with a clear "no hosts allowlisted" message until you opt hosts in via ``HEIM_MCP_HTTP_ALLOWLIST``.
...
the auto-follow is disabled. Set ``HEIM_MCP_HTTP_ALLOW_PRIVATE=1`` to intentionally reach an
...
caveat ``heim-mcp-web`` notes for its browser path) — we resolve, vet, and pin once, which is the
strong, practical guarantee.

Run standalone over stdio: ``heim-mcp-http`` (or ``python -m heim.mcp_servers.http``). Point heim at it
with ``heim mcp add http`` (or ``heim chat --mcp heim-mcp-http``).
"""
...
mcp: FastMCP = FastMCP("heim-http")


class Settings(BaseSettings):
    """Runtime config, from ``HEIM_MCP_HTTP_*`` env vars (all optional).

    ``allowlist`` is empty by default so the tool is fail-closed (deny all) until hosts are opted
    in. Comma-separated in the env var, e.g. ``HEIM_MCP_HTTP_ALLOWLIST=example.com,api.acme.io``.
    """

    model_config = SettingsConfigDict(env_prefix="HEIM_MCP_HTTP_")
    ...
    user_agent: str = "heim-mcp-http/0.1 (+https://github.com/winterop-com/heim)"
...
def _vet_hop(url: str) -> tuple[str, dict[str, str], dict[str, str]]:
    ...
    if not settings.allowlist:
        raise HttpToolError("no hosts allowlisted — set HEIM_MCP_HTTP_ALLOWLIST to opt hosts in (deny-all by default)")
...
async def http_get(url: str, headers: dict[str, str] | None = None) -> HttpResponse:
    ...
    so nothing is reachable until you configure ``HEIM_MCP_HTTP_ALLOWLIST``). Private/loopback/
    internal addresses are refused, and redirects are re-checked at every hop. The body is capped
    (``HEIM_MCP_HTTP_MAX_BYTES``) and marked ``truncated`` if it was cut. Blocks, timeouts, and
