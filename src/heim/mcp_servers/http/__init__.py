"""Heim MCP server: an SSRF-guarded, allowlisted HTTP fetch tool.

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) from ``app`` so both the
``heim-mcp-http`` console script and ``python -m heim.mcp_servers.http`` resolve against the package root.
