"""Heim MCP server: read-only file access (list / read / search) under one configured root.

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) so both the
``heim-mcp-files`` console script and ``python -m heim.mcp_servers.files`` resolve against the package root.
"""
...
def __getattr__(name: str) -> Any:  # noqa: D401
    ...
    (the console script or ``python -m``). Deferring it here keeps ``heim`` CLI startup fast.
    """
    if name in __all__:
