"""File access under one configured root — no MCP, no heim imports.
...
Writes are off unless ``HEIM_FILES_WRITABLE`` is set. Config via ``HEIM_FILES_*`` env.
"""
...
class FilesSettings(BaseSettings):
    """Config via ``HEIM_FILES_*`` env: which root to expose, whether writes are allowed, caps."""

    model_config = SettingsConfigDict(env_prefix="HEIM_FILES_", extra="ignore")

    root: Path = Path(".")  # HEIM_FILES_ROOT — the workspace the server exposes
    writable: bool = False  # HEIM_FILES_WRITABLE — opt in to write_file
...
def write_text(settings: FilesSettings, path: str, content: str) -> int:
    """Write ``content`` to ``path`` (creating parent dirs). Requires ``HEIM_FILES_WRITABLE``."""
    if not settings.writable:
        raise PermissionError("writes are disabled — set HEIM_FILES_WRITABLE=1 to allow them")
    target = safe_join(settings.root, path)
