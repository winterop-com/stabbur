"""Heim MCP server: core utility tools (text, encoding, hashing, JSON, math, stats).

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) so both the
``heim-mcp-utils`` console script and ``python -m heim.mcp_servers.utils`` resolve here.
"""
