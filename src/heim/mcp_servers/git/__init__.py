"""Heim MCP server: read-only git inspection, sandboxed to one configured repository.

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) from ``app`` so both the
``heim-mcp-git`` console script and ``python -m heim.mcp_servers.git`` resolve against the package root.
