"""Heim MCP server: persistent notes / key-value memory saved in the library.

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) from ``app`` so both
the ``heim-mcp-memory`` console script and ``python -m heim.mcp_servers.memory`` resolve against the
package root.
"""
