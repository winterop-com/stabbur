...
No MCP, no heim imports: just where the notes live and how they're read/written. The
...
Location (config via ``HEIM_*`` env, resolved by :class:`MemorySettings`):

* ``HEIM_MEMORY_DIR`` — explicit directory, if set;
* else ``<HEIM_LIBRARY_ROOT>/.heim/memory`` — so memory travels with the drive, next to
  the library's other metadata (``.heim/tags.json``), per heim's no-``~/.heim`` rule;
* else ``./.heim/memory`` — a project-local fallback when no library is configured.
"""
...
class MemorySettings(BaseSettings):
    """Where the notes file lives — resolved from ``HEIM_*`` env (or defaults)."""

    model_config = SettingsConfigDict(env_prefix="HEIM_", extra="ignore")

    memory_dir: Path | None = None  # HEIM_MEMORY_DIR — explicit override
    library_root: Path | None = None  # HEIM_LIBRARY_ROOT — memory travels with the library

    def notes_path(self) -> Path:
        """The JSON file the notes are stored in (directory created on first write)."""
        base = self.memory_dir or (
            self.library_root / ".heim" / "memory" if self.library_root else Path(".heim/memory")
        )
