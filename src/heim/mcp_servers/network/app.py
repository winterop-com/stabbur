...
Run standalone over stdio: ``heim-mcp-network`` (or ``python -m heim.mcp_servers.network``). Point heim at
it with ``heim chat --mcp heim-mcp-network`` (it's a bundled base server, added by default).
"""
...
mcp: FastMCP = FastMCP("heim-network")
