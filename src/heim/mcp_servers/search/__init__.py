"""Heim MCP server: web search returning titled results (title, url, snippet).

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) from ``app`` so
both the ``heim-mcp-search`` console script and ``python -m heim.mcp_servers.search`` resolve
