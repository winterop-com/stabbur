"""Heim MCP server: a locked-down Python code sandbox (calculator / scratchpad).

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) so both the
``heim-mcp-exec`` console script and ``python -m heim.mcp_servers.exec`` resolve against the package root.
"""
