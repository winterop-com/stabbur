"""Heim MCP server: read a web page in a headless browser and return readable Markdown.

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) from ``app`` so
both the ``heim-mcp-web`` console script and ``python -m heim.mcp_servers.web`` resolve against
the package root.
"""
