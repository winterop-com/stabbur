...
guarantee. Set ``HEIM_WEB_ALLOW_PRIVATE=1`` to intentionally read an internal/localhost
host. A per-fetch navigation timeout bounds hangs and the returned Markdown is
length-capped (``HEIM_WEB_MAX_CHARS``).
...
Run standalone over stdio: ``heim-mcp-web`` (or ``python -m heim.mcp_servers.web``). Point heim at it
with ``heim mcp add web`` (or ``heim chat --mcp heim-mcp-web``).
"""
...
mcp: FastMCP = FastMCP("heim-web")


class Settings(BaseSettings):
    """Runtime config, from ``HEIM_WEB_*`` env vars (all optional)."""

    model_config = SettingsConfigDict(env_prefix="HEIM_WEB_")
    ...
    # Playwright wait strategy; a Literal so an invalid HEIM_WEB_WAIT_UNTIL is rejected at startup.
    wait_until: Literal["load", "domcontentloaded", "networkidle", "commit"] = "load"
    allow_private: bool = False  # opt in to private/loopback hosts (internal servers)
    user_agent: str = "heim-mcp-web/0.1 (+https://github.com/winterop-com/heim)"
...
def _guard_url(url: str) -> None:
    ...
    ``HEIM_WEB_ALLOW_PRIVATE=1`` skips the address check (but the scheme is still enforced).
    """
    parsed = urlparse(url)
