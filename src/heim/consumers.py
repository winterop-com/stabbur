...
def lmstudio_models_dir() -> Path:
    """LM Studio's models directory (``HEIM_LMSTUDIO_MODELS_DIR`` / the detected default)."""
    from heim.config import get_settings  # noqa: PLC0415 - avoid an import cycle at module load
...
def uninstall_lmstudio(model: LibraryModel) -> InstallResult:
    ...
    Raises:
        RuntimeError: nothing heim-linked is there to remove (absent, or a real LM Studio copy).
    """
    link = lmstudio_models_dir() / model.name  # <base>/<publisher>/<repo>
    if not link.is_symlink():
        if link.exists():
            raise RuntimeError(f"{link} is a real LM Studio download, not a heim link — leaving it alone.")
...
def lmstudio_linked_names(library_roots: list[Path]) -> set[str]:
    ...
    That's exactly a heim install (:func:`install_lmstudio` symlinks the library copy in); a real
