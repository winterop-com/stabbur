...
/**
 * Canonical form of a kodo base URL for `apiFetch(baseUrl + path)`: trimmed, no
...
/**
 * Validate a kodo base URL. Returns null when acceptable, otherwise a
...
export function validateBaseUrl(raw: string): string | null {
  ...
  if (!value) return "Enter a kodo base URL (e.g. http://127.0.0.1:8000).";
  let url: URL;
  try {
    url = new URL(value);
  } catch {
    return "Enter a valid URL, e.g. http://127.0.0.1:8000 or https://kodo.example.com.";
  }
  ...
  if (url.protocol === "http:" && !isLoopback) {
    return "Remote kodo requires https (extension pages block mixed content). Use https:// or an SSH tunnel to 127.0.0.1.";
  }
  return null;
}
