...
// side panel, with the page-text toggle on, against a REAL `kodo serve` (locked
...
const UI_PORT = Number(process.env.KODO_UI_PORT ?? 4612);
...
test.describe.serial("extension UI spot-check against real kodo", () => {
  test("JSON extraction + summary + table through the side panel", async ({ context, extensionId }) => {
    ...
    let kodo: PromptServer | null = null;
    try {
      kodo = startPromptServer({ corsOrigin: `chrome-extension://${extensionId}`, port: UI_PORT });
      await waitForReady(kodo.baseUrl); // cold gemma load; project timeout covers it

      await seedSettings(context, extensionId, {
        baseUrl: kodo.baseUrl,
        token: "",
      ...
      if (kodo?.logPath) console.log(`[ui] kodo serve log tail:\n${kodo.tailLog(40)}`);
      throw err;
    } finally {
