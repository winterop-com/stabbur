// Live WRITE E2E: the extension panel driving a real `kodo serve` (locked model + DHIS2 CLI
...
//
// Because the assistant is readonly:false, kodo arms the per-write confirm gate: when the model
...
//   2. ask the model to CREATE a NUMBER data element `KODO_E2E_<rand>`; approve the confirm.
...
// All KODO_E2E_* objects this spec may create share this prefix, so the sweep can find strays.
const NAME_PREFIX = "KODO_E2E";
...
/** Best-effort: delete every KODO_E2E_* data element group (leaves no residue after a failed run). */
...
test.describe.serial("live extension WRITE against real kodo + local DHIS2", () => {
  ...
  test("creates a data element group through the confirm gate and read-back-verifies it (delete best-effort)", async ({
    ...
    try {
      ...
      // Boot kodo with the WRITE options: local_basic profile, mutable base_url, readonly:false
      ...
      if (server) console.log(`[live-write] kodo serve log tail:\n${server.tailLog(60)}`);
