...
`heim serve`. Built with WXT + React 19 + Tailwind v4.
...
## Flavors (generic vs heim for DHIS2)
...
| Env | `HEIM_FLAVOR` unset | `HEIM_FLAVOR=dhis2` |
| Output dir | `.output/chrome-mv3` | `.output/chrome-mv3-dhis2` |
| Manifest name | `heim` | `heim for DHIS2` |
| Description | generic side-panel copy | DHIS2 assistant copy |
| Icon | dark `k` mark | same mark + blue corner accent |
| Panel header | `heim` | `heim for DHIS2` |
| Disconnected hint | generic `heim serve` | mentions a DHIS2 project + docs link |

The flavor is selected by the `HEIM_FLAVOR` env var, read in `wxt.config.ts` (which
switches the manifest name/description/icon and the output dir) and baked into the
bundle via a Vite `define` (`__HEIM_FLAVOR__`). App code branches copy through
...
1. Run a heim server on a pinned port, e.g. `heim serve --port 8000`.
...
5. Click the heim toolbar icon to open the side panel.
...
this extension's origin. Add the following line to your heim config and restart
`heim serve` (the panel shows the exact line with a copy button):
...
The panel can point at a heim that is not on this machine, but an extension page
...
To connect to a remote heim:

1. **Serve heim behind TLS.** Terminate HTTPS in front of `heim serve` (a reverse
   proxy such as Caddy/nginx, or a tunnel that provides an `https://` URL). The
   panel talks to that `https://` origin.
2. **Set an auth token.** When heim binds a non-loopback interface it refuses to
   ...
   server answers `401`.
3. **Allow the extension origin.** Add this extension's origin to heim's
...
- **mock** (`bun run e2e`, or `make extension-e2e`) — fast and hermetic. Each spec
  starts a tiny in-process Node server that speaks the heim API contract
  (`/api/status`, `/api/load`, `/api/chat` SSE, `/api/assistant`) with scriptable
  scenarios, so it needs no heim, no model, and no network. Covers the connection
...
- **live** (`bun run e2e:live`, or `make extension-e2e-live`) — the real thing:
  it writes a throwaway heim project bound to a local GGUF model plus the DHIS2
  CLI bridge (pointed at the public **play** demo), runs `heim serve`, and drives
...
The live tier assumes `$HOME/.local/share/heim/library` contains
...
The panel imports the shared heim client and UI directly from `../frontend/src`
