...
`kodo serve`. Built with WXT + React 19 + Tailwind v4.
...
## Flavors (generic vs kodo for DHIS2)
...
| Env | `KODO_FLAVOR` unset | `KODO_FLAVOR=dhis2` |
| Output dir | `.output/chrome-mv3` | `.output/chrome-mv3-dhis2` |
| Manifest name | `kodo` | `kodo for DHIS2` |
| Description | generic side-panel copy | DHIS2 assistant copy |
| Icon | dark `k` mark | same mark + blue corner accent |
| Panel header | `kodo` | `kodo for DHIS2` |
| Disconnected hint | generic `kodo serve` | mentions a DHIS2 project + docs link |

The flavor is selected by the `KODO_FLAVOR` env var, read in `wxt.config.ts` (which
switches the manifest name/description/icon and the output dir) and baked into the
bundle via a Vite `define` (`__KODO_FLAVOR__`). App code branches copy through
...
1. Run a kodo server on a pinned port, e.g. `kodo serve --port 8000`.
...
5. Click the kodo toolbar icon to open the side panel.
...
requests (loading a model) are blocked by kodo's cross-site guard until you allow
this extension's origin. Add the following line to your kodo config and restart
`kodo serve` (the panel shows the exact line with a copy button):
...
The panel can point at a kodo that is not on this machine, but an extension page
...
To connect to a remote kodo:

1. **Serve kodo behind TLS.** Terminate HTTPS in front of `kodo serve` (a reverse
   proxy such as Caddy/nginx, or a tunnel that provides an `https://` URL). The
   panel talks to that `https://` origin.
2. **Set an auth token.** When kodo binds a non-loopback interface it refuses to
   ...
   server answers `401`.
3. **Allow the extension origin.** Add this extension's origin to kodo's
...
If you would rather not expose kodo publicly, forward it to loopback over SSH
...
- **mock** (`bun run e2e`, or `make extension-e2e`) — fast and hermetic. Each spec
  starts a tiny in-process Node server that speaks the kodo API contract
  (`/api/status`, `/api/load`, `/api/chat` SSE, `/api/assistant`) with scriptable
  scenarios, so it needs no kodo, no model, and no network. Covers the connection
...
- **live** (`bun run e2e:live`, or `make extension-e2e-live`) — the real thing:
  it writes a throwaway kodo project bound to a local GGUF model plus the DHIS2
  CLI bridge (pointed at the public **play** demo), runs `kodo serve`, and drives
...
The live tier assumes `$HOME/.local/share/kodo/library` contains
...
The panel imports the shared kodo client and UI directly from `../frontend/src`
