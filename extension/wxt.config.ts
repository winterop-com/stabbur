...
// Build flavor, selected by the HEIM_FLAVOR env var (see lib/flavor.ts). "dhis2" ships the
// "heim for DHIS2" build (branded name/description + DHIS2 copy) into its own output dir so
...
const FLAVOR = process.env.HEIM_FLAVOR === "dhis2" ? "dhis2" : "generic";
const IS_DHIS2 = FLAVOR === "dhis2";

// TEST-ONLY build variant (HEIM_E2E=1). It lands in its own output dir (`.output/chrome-mv3-e2e`)
...
const IS_E2E = process.env.HEIM_E2E === "1";
...
const MANIFEST_NAME = IS_DHIS2 ? "heim for DHIS2" : "heim";
const MANIFEST_DESCRIPTION = IS_DHIS2
  ...
  : "Side panel for your local heim assistant";
...
export default defineConfig({
  ...
  hooks: {
    "vite:build:extendConfig": (_entrypoints, config) => {
      config.plugins ??= [];
      config.plugins.push({
        name: "heim:at-alias-to-frontend",
  ...
  manifest: {
    ...
    // Requested on demand during the session-fallback bind consent (user gesture), so heim can read
