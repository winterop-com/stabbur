...
`sb chat` talks to that server's `/v1` behind a full-screen TUI; `sb serve`
...
```bash
sb serve --ui                       # browse + chat in the browser (loads models on demand)
sb serve --model <name>             # lock the server to one model; stable /v1 endpoint
sb serve --model <name> --port 9000 # pin the port
```
...
```bash
sb chat <name>                      # clean streaming REPL (/exit to quit)
sb chat <name> -p "Summarize MoE"   # one-shot: prints only the answer (pipeable)
sb chat <name> -p "..." -n 256      # cap generated tokens
```

The `-p/--prompt` one-shot mirrors `claude -p` — clean stdout for scripting:

```bash
sb chat gemma-4-12B-it-QAT-GGUF -p "Say hi" 2>/dev/null | tee out.txt
```
...
library but is in a source store, the error tells you the `sb library pull` to run.
