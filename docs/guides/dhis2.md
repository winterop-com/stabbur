...
stabbur's north star is a **local, self-hosted DHIS2 assistant** — your own model driving
...
|--------|-------|----------|
| **`dhis2w-mcp-bridge`** | 1 (`dhis2_cli`) | **Smaller local models.** One tool that runs any `d2w` command; the model composes CLI argv. stabbur's default. |
...
- "Create a new data element group called 'stabbur test group'."
