...
You don't have to go through `stabbur` to run a library model — the files are
...
```bash
GGUF="$STABBUR_LIBRARY_ROOT/gguf/lmstudio-community/gemma-4-12B-it-QAT-GGUF/gemma-4-12B-it-QAT-Q4_0.gguf"
...
```bash
MLX="$STABBUR_LIBRARY_ROOT/mlx/lmstudio-community/Qwen3.5-4B-MLX-4bit"
