...
import pytest
from kodo_mcp_weather_yr.core import Location, WeatherSettings, fetch_forecast
...
def test_settings_default_user_agent() -> None:
    assert "kodo-mcp-weather-yr" in WeatherSettings().user_agent


def test_fetch_forecast_parses(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("kodo_mcp_weather_yr.core.httpx.Client", _FakeClient)
