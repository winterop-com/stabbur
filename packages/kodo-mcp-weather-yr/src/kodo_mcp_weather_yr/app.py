...
fixed so there is no arbitrary-fetch surface. See :mod:`kodo_mcp_weather_yr.core`.

Run standalone over stdio: ``kodo-mcp-weather-yr`` (or ``python -m kodo_mcp_weather_yr``). Point
kodo at it with ``kodo chat --mcp weather-yr`` or a ``[[mcp]]`` block.
"""
...
from kodo_mcp_weather_yr.core import Location, WeatherSettings, fetch_forecast, geocode

mcp: FastMCP = FastMCP("kodo-weather-yr")
