"""Weather lookups against the free met.no (yr.no) APIs — no MCP, no kodo imports.
...
(override with ``KODO_WEATHER_USER_AGENT``) and coordinates are truncated to 4 decimals to aid
their caching.
"""
...
class WeatherSettings(BaseSettings):
    """Config via ``KODO_WEATHER_*`` env. The User-Agent identifies this app to met.no/Nominatim."""

    model_config = SettingsConfigDict(env_prefix="KODO_WEATHER_", extra="ignore")

    user_agent: str = "kodo-mcp-weather-yr/0.1 github.com/winterop-com/kodo"
