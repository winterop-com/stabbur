...
An [MCP](https://modelcontextprotocol.io) server that **reads a web page in a
headless browser and returns readable Markdown**. It renders the page with
Playwright (Chromium) — so JavaScript-heavy pages work — then extracts the main
content with [trafilatura](https://trafilatura.readthedocs.io) (nav/ads/boilerplate
removed). Kodo is the MCP *client*; this is one of the servers it can spawn so a model
can ground itself on a page ("read this URL").
