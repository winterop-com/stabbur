"""Kodo MCP server: read-only git inspection, sandboxed to one configured repository.

Re-exports the FastMCP app (``mcp``) and stdio entry point (``main``) from ``app`` so both the
``kodo-mcp-git`` console script and ``python -m kodo_mcp_git`` resolve against the package root.
