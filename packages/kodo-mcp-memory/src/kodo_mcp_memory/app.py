...
lives in the library by default (``<KODO_LIBRARY_ROOT>/.kodo/memory/notes.json``), so it
travels with the drive — see :mod:`kodo_mcp_memory.core` for the location rules.

Run standalone over stdio: ``kodo-mcp-memory`` (or ``python -m kodo_mcp_memory``). Point kodo
at it with ``kodo chat --mcp memory`` or a ``[[mcp]]`` block.
"""
...
mcp: FastMCP = FastMCP("kodo-memory")


def _store() -> MemoryStore:
    """Build the store from the current environment (read per call so KODO_* is respected)."""
