...
Lets an assistant browse, read, and search a project directory (the root, ``KODO_FILES_ROOT``,
defaults to the current directory). Every path is contained to the root by ``safe_join`` — no
absolute paths, no ``..`` escapes. Writes are off unless ``KODO_FILES_WRITABLE`` is set.

Run standalone over stdio: ``kodo-mcp-files`` (or ``python -m kodo_mcp_files``). Point kodo at it
with ``kodo chat --mcp files`` or a ``[[mcp]]`` block.
"""
...
from kodo_mcp_files.core import FilesSettings, list_dir, read_text, search, write_text

mcp: FastMCP = FastMCP("kodo-files")
...
def write_file(path: str, content: str) -> dict[str, Any]:
    """Write `content` to a file under the root. Disabled unless KODO_FILES_WRITABLE is set."""
