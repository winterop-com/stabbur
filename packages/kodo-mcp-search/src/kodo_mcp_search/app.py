...
title, URL, and snippet — for a model to then ``read_url`` (pairs with ``kodo-mcp-web``).

Backends (``KODO_SEARCH_BACKEND``, default ``auto``):

* ``duckduckgo`` — no API key; scrapes the DuckDuckGo HTML endpoint. The zero-config default.
* ``brave`` — the Brave Search API; needs ``KODO_SEARCH_BRAVE_KEY``.
* ``exa`` — the Exa API; needs ``KODO_SEARCH_EXA_KEY``.
...
Run standalone over stdio: ``kodo-mcp-search`` (or ``python -m kodo_mcp_search``). Point kodo
at it with ``kodo mcp add search`` (or ``kodo chat --mcp kodo-mcp-search``).
"""
...
mcp: FastMCP = FastMCP("kodo-search")


class Settings(BaseSettings):
    """Runtime config, from ``KODO_SEARCH_*`` env vars (all optional)."""

    model_config = SettingsConfigDict(env_prefix="KODO_SEARCH_")
    ...
    user_agent: str = "Mozilla/5.0 (compatible; kodo-mcp-search/0.1; +https://github.com/winterop-com/kodo)"
...
async def _search_brave(query: str, limit: int) -> list[SearchResult]:
    if not settings.brave_key:
        raise ValueError("the Brave backend needs KODO_SEARCH_BRAVE_KEY (or use the default duckduckgo backend)")
    async with httpx.AsyncClient(
        timeout=settings.timeout_seconds,
...
async def _search_exa(query: str, limit: int) -> list[SearchResult]:
    if not settings.exa_key:
        raise ValueError("the Exa backend needs KODO_SEARCH_EXA_KEY (or use the default duckduckgo backend)")
    async with httpx.AsyncClient(
        timeout=settings.timeout_seconds,
...
async def search(query: str, max_results: int = 0) -> str:
    ...
    Brave/Exa API when a ``KODO_SEARCH_*`` key is configured.
    """
    limit = max_results if max_results and max_results > 0 else settings.max_results
    backend = resolve_backend()
    fn = _BACKENDS.get(backend)
    if fn is None:
        raise ValueError(f"unknown KODO_SEARCH_BACKEND {backend!r}; use one of: auto, duckduckgo, brave, exa")
    try:
