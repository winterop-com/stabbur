...
its output back. The sandbox is :mod:`kodo_sandbox` (shared with the benchmark). Requires a
running Docker daemon; without one the tool returns a clear error instead of executing.

Run standalone over stdio: ``kodo-mcp-exec`` (or ``python -m kodo_mcp_exec``). Point kodo at it
with ``kodo chat --mcp exec`` or a ``[[mcp]]`` block.
"""
...
from kodo_sandbox import DockerError, run_code

mcp: FastMCP = FastMCP("kodo-exec")
