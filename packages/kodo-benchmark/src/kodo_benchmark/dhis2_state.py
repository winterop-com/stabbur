"""Live DHIS2 read-back + KODO_ residue sweep for the stateful write benchmark.
...
:func:`kodo_benchmark.core.score_tool_stateful`). This module holds the concrete DHIS2 client
(:class:`Dhis2StateReader`, a :class:`~kodo_benchmark.core.StateReader`) plus the between-models
residue sweep that deletes any ``KODO_``-prefixed leftovers so a model that abandons a lifecycle
...
def load_profile(name: str | None, profiles_path: Path | None = None) -> Dhis2Profile | None:
    ...
    Environment overrides (win over the file so CI can point elsewhere): ``KODO_BENCH_DHIS2_URL``,
    ``KODO_BENCH_DHIS2_USER``, ``KODO_BENCH_DHIS2_PASSWORD``. Otherwise reads
    ...
    """
    env_url = os.environ.get("KODO_BENCH_DHIS2_URL")
    if env_url:
        return Dhis2Profile(
            base_url=env_url,
            username=os.environ.get("KODO_BENCH_DHIS2_USER", "admin"),
            password=os.environ.get("KODO_BENCH_DHIS2_PASSWORD", "district"),
        )
    if not name:
        return None
...
class Dhis2StateReader:
    """A live DHIS2 read/delete client for verifying and cleaning ``KODO_`` write residue.
    ...
    name is not ``KODO_``-prefixed, so the sweep can only ever touch benchmark test objects.
    """
    ...
    def sweep(self, resource: str, prefix: str = KODO_PREFIX) -> list[str]:
        ...
        targets within the ``KODO_`` prefix (the shipped suite does).
        """
        # Hard invariant: only ever sweep a KODO_-scoped prefix. An empty / non-KODO prefix (a future
        # caller's bug) would make ``startswith`` trivially true and turn the safety net into a no-op.
        if not prefix.startswith(KODO_PREFIX):
            return []
...
def sweep_residue(reader: Dhis2StateReader, resources: list[str], prefix: str = KODO_PREFIX) -> dict[str, list[str]]:
    """Sweep ``KODO_`` residue across ``resources``; return ``{resource: [deleted uids]}``.
