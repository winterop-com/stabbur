"""CLI one-shot (`kodo chat -p`) confirmation-gate wiring.

A write-enabled project assistant makes ``kodo chat -p`` gate non-read-only tool calls. The
...
from kodo import agent, capabilities, cli, tools
