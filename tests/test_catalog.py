...
`heim mcp add <name>` resolves against the curated catalog, and scaffolding writes each
...
from heim.mcp_catalog import CURATED
from heim.project.templates import TEMPLATES


def test_playwright_is_addable_via_mcp_add() -> None:
    # heim mcp add resolves names against CURATED; playwright must be there with a runnable command.
...
def test_all_curated_and_template_commands_parse() -> None:
    ...
    # so `heim mcp add` and scaffolding never write a broken .mcp.json.
