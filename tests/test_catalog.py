...
`stabbur mcp add <name>` resolves against the curated catalog, and scaffolding writes each
...
from stabbur.mcp_catalog import CURATED
from stabbur.project.templates import TEMPLATES


def test_playwright_is_addable_via_mcp_add() -> None:
    # stabbur mcp add resolves names against CURATED; playwright must be there with a runnable command.
...
def test_all_curated_and_template_commands_parse() -> None:
    ...
    # so `stabbur mcp add` and scaffolding never write a broken .mcp.json.
