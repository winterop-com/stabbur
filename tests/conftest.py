...
thus ``heim.cli``'s module-level Rich ``Console()``) is imported. CLI tests
...
# (e.g. a copy-paste ``ollama run`` command straddling the fold). Set before ``heim.cli``'s
...
# tests inherit whatever ``HEIM_LIBRARY_ROOT`` the developer's ``.env`` supplies —
...
os.environ.setdefault("HEIM_LIBRARY_ROOT", tempfile.mkdtemp(prefix="heim-test-library-"))

# Point the machine config (heim.userconfig, read as the lowest-priority Settings source) at a
# throwaway XDG dir so the suite never reads the developer's real ~/.config/heim/config.toml —
