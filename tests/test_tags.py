"""Tests for the user-tag store (stabbur.tags)."""

from pathlib import Path

from stabbur import tags
...
def test_load_missing_or_corrupt_is_empty(tmp_path: Path) -> None:
    ...
    path = tmp_path / ".stabbur" / "tags.json"
...
def test_load_registry_ignores_garbage(tmp_path: Path) -> None:
    ...
    (tmp_path / ".stabbur" / "tag-registry.json").write_text('{"ok": {"color": "#fff"}, "bad": 5}')
    reg = tags.load_registry(tmp_path)
