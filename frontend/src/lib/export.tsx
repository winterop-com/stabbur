...
export function exportConversationMarkdown(conv: Conversation, model: string | null): void {
  ...
  downloadFile(`kodo-${slug(conv.title)}-${isoDate(Date.now())}.md`, md, "text/markdown;charset=utf-8");
}
...
export async function exportConversationPdf(conv: Conversation, model: string | null): Promise<void> {
  ...
  if (!w) {
    ...
    downloadFile(`kodo-${slug(conv.title)}-${isoDate(Date.now())}.html`, html, "text/html;charset=utf-8");
    return;
  }
  w.document.open();
